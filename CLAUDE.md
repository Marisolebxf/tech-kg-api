...
- `init_graph_schema.py` — `CREATE SPACE techkg` + Scholar/Organization/`EMPLOYED_BY` DDL via nGQL.
...
- The `engine` / `graph` networks are **external** (`tech-kg-engine_default`, `trs-graph-service_default`) — `docker compose up` fails if they don't exist. Create them (`docker network create ...`) or bring up the engine/trs-graph compose projects first.
