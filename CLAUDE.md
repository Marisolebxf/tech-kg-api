...
- **Manual review** (`/api/v1/manual-review`, admin-gated; `/api/v1/internal/manual-reviews`, service auth) — production review service (`service/manual_review_production.py`) with a graph-build handoff boundary; 9 tables in `db_model/manual_review.py` (business DB). The **internal endpoints are called by the graph-build service** (not yet built in prod; test double `script/run_graph_build_double.py`, `REVIEW_RERUN_MODE` defaults to `mock`). The review worker (`script/run_manual_review_worker.py`, not in compose) calls `process_outbox()` + `reclaim_expired()` **in-process every 2s** — it does not call HTTP. Details: `docs/人工审核模块.md`.
