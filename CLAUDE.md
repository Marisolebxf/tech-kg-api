...
- **Workflow system** (`/api/v1/workflow-system`, admin-gated) — Temporal-backed workflow execution + a **MySQL control plane** (`techkg_control` on the temporal-mysql instance; `service/workflow_repository.py` + `infra/workflow_mysql.py`, env `WORKFLOW_MYSQL_*` — the old SQLite/`WORKFLOW_DATABASE_PATH` path no longer exists in code). Definitions are script-based (`kg.custom.python`/`steps`/`chain`) plus a declarative type whose steps run accounting stubs; they do **not** compose OperatorRegistry operators (parallel subsystems). The only Temporal worker is the `temporal-worker` container running `script/run_temporal_worker.py` (Temporal at `TEMPORAL_ADDRESS`; no in-process api worker). All workflows share one task queue (`TEMPORAL_TASK_QUEUE`, default `tech-kg-workflows`); activity concurrency/retry are capped (retry-storm protection, commit `932fc82`) to protect the trs-graph session pool. Details: `docs/工作流系统模块.md`.
- **Operator registry** (`service/operator_registry.py`) — thread-safe, hot-reloadable Python operator registry. Built-ins in `service/operator_builtins.py`; user operators loaded from `operators/user/` and persisted to S3 (`OPERATOR_S3_*`; no MySQL table). Each operator follows `list[dict] -> operator(data, ctx) -> list[dict]`. `main.py` lifespan calls `REGISTRY.initialize_store()` + `start_watcher()`. Consumed via the HTTP `invoke` endpoint and the scholar ETL registration line (`script/register_scholar_operators.py`) — **workflow steps do not reference operators**. Uploads are syntax/signature-checked but NOT LLM-security-reviewed (that is schema-extraction-scripts only); the security boundary is admin-only trust. Details: `docs/算子注册模块.md`.
