...
Monorepo: `backend/` (Python FastAPI) + `frontend/` (Vue 3 + TS + Vite). A top-level `docker-compose.yml` builds and runs both, plus the supporting infra services (Milvus+etcd, one shared RustFS S3 for schema scripts / Milvus internal storage — no MinIO, m3e-embedding, auth-redis, Temporal).
...
**Gitignored / local-only — do not add to git:** `backend/app/`, root `app/`, `docs/superpowers/`. The `backend/config/*.yml` files are legacy and **not loaded** by the live app — config comes from env vars / `.env` via `python-dotenv`.
...
- **CI (`.github/workflows/`) runs two workflows on PRs to `main`:** `ci.yml` (ruff format/check + `pytest -m "not external"`) and `docker-test.yml` (builds the compose stack, waits for `/health` on port 8001, runs `pytest tests/integration -m "not external"` inside the `api` container, then tears down).
...
The backend and all infra (MySQL, RustFS, Temporal, Milvus, redis, trs-graph) run **only in Docker** — the host has none of them. **Never run backend-dependent tests on the host** (they fail with e.g. `Can't connect to MySQL server on 'temporal-mysql'`, DNS unresolvable). Verified in-container commands:
...
# Frontend unit tests — inside the web image's builder stage:
...
docker run --rm tech-kg-dev2-frontend-test pnpm vitest run
```
...
- `frontend/src/__tests__/review-full-integration.spec.ts` (environment-gated full-loop test) was **deleted** with the graph-build handoff channel (2026-09-15); no vitest `--exclude` is needed anymore.
...
- **`biz/handler/`** — FastAPI `APIRouter`s (the only place routes are defined). Routers are mounted by `biz/router/register.py` under `/api/v1`. Routers split into two groups: **protected** (depend on `require_authenticated_user`) and **admin** (additionally `require_platform_admin`). (A third "internal" group existed for the graph-build handoff; removed 2026-09-15.) Handlers are thin: parse request, call an `application` object, return response.
- **`biz/dependencies/auth.py`** — auth dependency wiring (see Auth below).
- **`biz/schemas/`** — Pydantic v2 request/response models, one file per module. `common.py` holds the `ApiResponse` envelope. `text_rules.py` is the shared text-input validation (64-char max + character whitelist via `check_text()` / pattern constants; `*_QUERY_PATTERN` for GET `Query(pattern=...)`); all human-typed text fields across module schemas go through it — reuse it for new request models rather than inventing per-module rules.
...
- **`infra/`** — infrastructure singletons, all reading config from env vars / `.env` via `python-dotenv`:
  ...
  - `s3.py` — generic boto3 S3 wrapper (MinIO-compatible).
...
- **`idl/`**, **`middleware/`**, **`utils/`**, **`static/`**, **`var/`** — interface definitions, middleware stubs, shared utils, swagger static assets, runtime state (patent index state; `var/tech-kg-workflows.db` is a legacy SQLite file from before the workflow control plane moved to MySQL `techkg_control`).
...
- **Schema management** (`/api/v1/schema-management`, admin-gated) — entity/relation schema catalog synced to Nebula TAG/EDGE DDL on create/alter; extraction scripts stored in S3 (`SCHEMA_S3_*`), syntax+entry-checked in code and **LLM-reviewed** by `service/script_security.py` (not an AST allowlist) before saving. `SCHEMA_AUTO_INIT=true` seeds the catalog on boot. Details: `docs/Schema管理模块.md`.
- **平台喂数批次抽取**（`kg.schema.extract`，`service/temporal_workflows.py:SchemaExtractWorkflow`）— schema 抽取主通道：来源绑定（`GraphSchemaSource`，复杂 SQL 走 `query_sql` 列）分批读 → 转换脚本 `transform(payload)` 只输出 `{entities|edges, failures}` JSON → 平台写图/消歧/索引/推游标。脚本也可做多步转换：推荐 `from kg_sdk import step` 后在顶层函数上标 `@step` / `@step("id")`（步顺序 = 函数源码出现顺序；`sdk/kg_sdk.py` 的装饰器运行时恒等），兼容旧顶层 `STEPS` 清单；两种声明均与 `transform` 互斥，`service/script_steps.py` 纯 AST 静态解析（单 `transform` = 单步特例；第 N>1 步 payload 为 `{"input": 上一步输出, ...}`，每步独立 Temporal 重试，任意步出记录即写图，水位仍整链推进）。读取模式：querySql 绑定走水位/pk keyset（合成唯一 pk）；普通表走 LIMIT/OFFSET（主键不唯一，与旧脚本同语义），注册器 `script/register_platform_extraction.py` 按 information_schema 自动探测 pk/时间列。**实体写图必须走 nGQL `INSERT VERTEX`**（trs-graph `/nodes/merge` 会把 id/name/vid 从属性剥离，schema DDL 的 NOT NULL id/name 会 400）。逐行失败 → T_EXTRACT_FAIL 审核 case（队列 category=C）；点重跑（`POST /manual-reviews/production/rerun-extract-failures`）→ 新执行 `triggerSource=RERUN`（MANUAL/SCHEDULE/RERUN 三类在任务详情执行历史同列展示）。任务类型 `extract`（schemaId+batchSize+cron）与 `chain`（多脚本串行：schemaIds 2–20 个严格串行逐 Schema 抽取，`kg.schema.extract.chain` 子类复用单 Schema 全链路，任一环失败整链 FAILED，每环水位/索引/审核语义不变；详情页每个 Schema 一个抽屉，抽屉内为脚本转换步）。索引重建（`EntitySearchService.reindex`）失败降级不拖垮抽取。**批载荷 S3 中转（claim-check，`SCHEMA_EXTRACT_PAYLOAD_S3_ENABLED` 默认关，dev2 默认开）**：批行/转换输出/步间透传/失败清单归档 RustFS S3（`service/extract_payload_store.py`，key `runs/{wf}/{run}/payloads/{stepId}/{batchIdx}/{artifact}.json.gz`，worker 每日清理 + bucket lifecycle），事件历史只进 key+计数，activity 内下载后组装的 payload 与内联路径逐字节等价（脚本无感）；开时批预算 8MB（大文本行不再按 512KB 折半），workflow 按「历史里记录的结果形状」分支（chunks/outKey/recordsKey/failuresKey），翻 flag 无需排空在飞 run。e2e 驱动：根目录 `dev2_extract_e2e.py`（含大文本行不折半断言）。
- **Workflow system** (`/api/v1/workflow-system`, admin-gated) — Temporal-backed workflow execution + a **MySQL control plane** (`techkg_control` on the temporal-mysql instance; `service/workflow_repository.py` + `infra/workflow_mysql.py`, env `WORKFLOW_MYSQL_*` — the old SQLite/`WORKFLOW_DATABASE_PATH` path no longer exists in code). Definitions are the declarative type (`kg.custom.configurable`, steps run inline accounting stubs) plus the schema-extract synthetic type (`kg.schema.extract`) — the standalone script-upload channels and the operator registry were removed (2026-09-14 D2/D4). The only Temporal worker is the `temporal-worker` container running `script/run_temporal_worker.py` (Temporal at `TEMPORAL_ADDRESS`; no in-process api worker). All workflows share one task queue (`TEMPORAL_TASK_QUEUE`, default `tech-kg-workflows`); activity concurrency/retry are capped (retry-storm protection, commit `932fc82`) to protect the trs-graph session pool. Details: `docs/工作流系统模块.md`.
- **Manual review** (`/api/v1/manual-review`, admin-gated; gateway-identity auth via `get_review_identity`) — production review service (`service/manual_review_production.py`); 5 tables in `db_model/manual_review.py` (business DB). Three live templates only: **T_DIRECT** (accept → direct nGQL write, bypasses claim/submit), **T_LINK** (same-name entity-alignment verdict; merge execution lands in the vector-alignment merge engine, later task), **T_EXTRACT_FAIL** (rerun via `POST /manual-reviews/production/rerun-extract-failures` → new execution `triggerSource=RERUN`). The graph-build handoff channel (internal ingress, correction/outbox, review worker) and 6 dormant templates were removed 2026-09-15; verdicts land in-repo. Details: `docs/人工审核模块.md`.
...
- **LLM** (`infra/llm.py`) — `get_llm_client()` is a process singleton; returns `None` when `LLM_API_KEY`/`ZHIPUAI_API_KEY` is unset (caller degrades). Default model `glm-5.3-flash`. `synthesize()` returns `None` on any error. Patent hybrid search uses a separate m3e embedding service (`PATENT_EMBEDDING_BASE_URL`, default `http://m3e-embedding:8010/v1`).
...
- `script/run_temporal_worker.py` — the only long-running worker (Temporal task queue; the manual-review worker was removed with the graph-build handoff channel, 2026-09-15).
...
- **API baseURL must stay a relative fallback**: `src/api/http.ts` (and the SSE path in `schemaManagement.ts`) use `import.meta.env.VITE_API_BASE || './api'`. The relative `./api` makes one image work both at root path and behind a gateway subpath (e.g. `https://host/bkg_zp/` — gateway strips the prefix, nginx `/api/` proxy hits). `VITE_API_BASE` (build arg / compose env, default empty) exists only for explicit absolute overrides; do not change the fallback back to `/api`.
...
`docker-compose.yml` runs the two app services plus supporting infra (Milvus + etcd, one shared `operator-rustfs` S3 for schema scripts / Milvus internal storage, m3e-embedding, auth-redis, Temporal + temporal-mysql):
