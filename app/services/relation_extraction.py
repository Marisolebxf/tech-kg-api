...
class RuleBasedExtractor:
    ...
    def _recognize_entities(self, text: str) -> list[Entity]:
        ...
        context_patterns = [
            # 创立
            (
                r"([\u4e00-\u9fff]{2,3}?)[，,]?\s*(?:创立|创办|创建)\s*(?:了\s*)?([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))",
                "PERSON",
                "ORG",
            ),
            # 担任
            (
                r"([\u4e00-\u9fff]{2,3}?)[，,]?\s*(?:担任|出任|就任|现任)\s*(?:了?\s*)?([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))",
                "PERSON",
                "ORG",
            ),
            # 出生于
            (
                r"([\u4e00-\u9fff]{2,3}?)[，,]?\s*(?:出生于|生于|来自|籍贯)\s*([\u4e00-\u9fff]{1,8}(?:省|市|区|县|州|国))",
                "PERSON",
                "LOC",
            ),
            # 位于
            (
                r"([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))[，,]?\s*(?:位于|坐落于|总部在|设于|地处)\s*([\u4e00-\u9fff]{1,8}(?:省|市|区|县|州|国))",
                "ORG",
                "LOC",
            ),
            # 投资
            (
                r"([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))[，,]?\s*(?:投资|注资|领投)\s*(?:了\s*)?([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))",
                "ORG",
                "ORG",
            ),
            # 收购
            (
                r"([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))[，,]?\s*(?:收购|并购|兼并|买下|购入)\s*(?:了\s*)?([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))",
                "ORG",
                "ORG",
            ),
            # 合作
            (
                r"([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))[，,]?\s*(?:与|和)\s*([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))",
                "ORG",
                "ORG",
            ),
            # 产品
            (
                r"([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))[，,]?\s*(?:推出|发布|研发|开发|研制)\s*(?:了\s*)?([\u4e00-\u9fffA-Za-z0-9]{2,10}(?:大模型|操作系统|系统|平台|手机|汽车|芯片))",
                "ORG",
                "PRODUCT",
            ),
        ]
        ...
        general_patterns = [
            (
                r"[\u4e00-\u9fffA-Za-z]{1,6}(?:有限公司|股份公司|集团|公司|银行|基金|资本|研究院)",
                "ORG",
            ),
    ...
    def extract(self, text: str, entities: list[Entity] | None = None) -> list[Relation]:
        ...
        text_level_patterns = [
            # 创立
            (
                r"([\u4e00-\u9fff]{2,4}?)[，,]?\s*(?:创立|创办|创建)\s*(?:了\s*)?([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))",
                "创立",
                "PERSON",
                "ORG",
            ),
            # 就职
            (
                r"([\u4e00-\u9fff]{2,4}?)[，,]?\s*(?:担任|出任|就任|现任)\s*(?:了?\s*)?([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))\s*(?:的\s*)?(?:CEO|CFO|CTO|COO|董事长|总裁|总经理|总监|创始人|联合创始人|首席\w+官|VP|副总裁)",
                "就职于",
                "PERSON",
                "ORG",
            ),
            # 位于
            (
                r"([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))[，,]?\s*(?:位于|坐落于|总部在|设于|地处)\s*([\u4e00-\u9fff]{1,6}(?:省|市|区|县|州|国))",
                "位于",
                "ORG",
                "LOC",
            ),
            # 籍贯
            (
                r"([\u4e00-\u9fff]{2,4}?)[，,]?\s*(?:出生于|生于|来自|籍贯)\s*([\u4e00-\u9fff]{1,6}(?:省|市|区|县|州|国))",
                "籍贯",
                "PERSON",
                "LOC",
            ),
            ...
            # 合作
            (
                r"([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))[，,]?\s*(?:与|和)\s*([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))\s*(?:合作|达成合作|签署合作|建立合作|战略合作)",
                "合作",
                "ORG",
                "ORG",
            ),
            # 合作(兜底)
            (
                r"([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))[，,]?\s*(?:与|和)\s*([\u4e00-\u9fffA-Za-z]{2,8}?)(?:\s+|在|，|,)\s*(?:\w+领域)?\s*(?:合作|达成合作|签署合作|建立合作|战略合作)",
                "合作",
                "ORG",
                "ORG",
            ),
            # 联合发布
            (
                r"([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))[，,]?\s*(?:与|和)\s*([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))\s*(?:联合|共同)\s*(?:发布|开发|推出|研发)",
                "合作",
                "ORG",
                "ORG",
            ),
            # 产品
            (
                r"([\u4e00-\u9fffA-Za-z]{1,15}(?:有限公司|股份公司|集团|公司|银行|基金|资本))[，,]?\s*(?:推出|发布|研发|开发|研制)\s*(?:了\s*)?([\u4e00-\u9fffA-Za-z0-9]{2,10}(?:大模型|操作系统|系统|平台|手机|汽车|芯片|产品))",
                "产品",
                "ORG",
                "PRODUCT",
            ),
...
class LLMExtractor:
    ...
    def __init__(
        self, api_key: str | None = None, base_url: str | None = None, model: str = "glm-4-flash"
    ):
        self.model = model
