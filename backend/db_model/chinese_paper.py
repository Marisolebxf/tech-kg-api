...
class DwdZhJournal(Base):
    ...
    id = Column('id', BigInteger(), primary_key=True, nullable=False, comment='期刊记录的唯一标识。')
    ...
    publication_type = Column('publication_type', String(255), nullable=True, comment='出版物类别，例如期刊，会议等')
    ...
    zh_description = Column('zh_description', Text(), nullable=True, comment='期刊的中文简介或说明。')
    format = Column('format', String(63), nullable=True, comment='期刊的版面开本规格。')
    founding_time = Column('founding_time', Integer(), nullable=True, comment='期刊首次创办或发行的时间。')
    language_classify = Column('language_classify', SmallInteger(), nullable=True, comment='期刊出版使用的语言类型。')
    ...
    postal_code = Column('postal_code', String(32), nullable=True, comment='期刊邮政发行使用的代号。')
    chief_editor = Column('chief_editor', String(128), nullable=True, comment='期刊当前或记录中的主编信息。')
    organizer = Column('organizer', String(1024), nullable=True, comment='负责主办该期刊的单位名称。')
    publisher_place = Column('publisher_place', String(64), nullable=True, comment='期刊出版发行的地点。')
    publication_cycle = Column('publication_cycle', String(64), nullable=True, comment='期刊的出版频率或发行周期。')
    award = Column('award', Text(), nullable=True, comment='期刊获得的奖项或荣誉信息。')
    cite_nums = Column('cite_nums', Integer(), nullable=True, comment='期刊或文献的累计被引用次数。')
    ...
    annual_publication = Column('annual_publication', Integer(), nullable=True, comment='期刊每年发表文章的数量。')
    impact_factor = Column('impact_factor', Numeric(20, 10), nullable=True, comment='期刊的影响因子指标。')
    open_access = Column('open_access', SmallInteger(), nullable=True, comment='标识期刊或论文是否为开放获取。')
    ...
    sub_quartile = Column('sub_quartile', SmallInteger(), nullable=True, comment='期刊在相关评价体系中的分区信息。')
    classify_list = Column('classify_list', Text(), nullable=True, comment='文献或期刊对应的学科分类编号。')
...
class DwdZhPaper(Base):
    ...
    cover_date_start = Column('cover_date_start', String(255), nullable=True, comment='文献正式发表的日期。')
