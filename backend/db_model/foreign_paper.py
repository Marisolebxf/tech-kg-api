...
class DwdEnAuthor(Base):
    ...
    correspond = Column(
        "correspond", SmallInteger(), nullable=True, comment="是否为通讯作者。否=0，是=1，未知=Null"
    )
...
class DwdEnJournal(Base):
    ...
    id = Column(
        ...
        primary_key=True,
        nullable=False,
        comment="期刊、会议或预印本记录的唯一标识。",
    )
    issn = Column("issn", String(16), nullable=True, comment="期刊的国际标准连续出版物编号。")
    zh_name = Column(
        "zh_name", String(1024), nullable=True, comment="期刊、顶会或预印本平台的中文名称。"
    )
    en_name = Column(
        "en_name", String(1024), nullable=True, comment="期刊、顶会或预印本平台的英文名称。"
    )
    name_abbr = Column(
        "name_abbr", String(255), nullable=True, comment="期刊、会议或预印本名称的简称或缩写。"
    )
    publication_type = Column(
        "publication_type",
        String(255),
        nullable=True,
        comment="标识出版载体的类型，如期刊、会议或预印本等。journal，conference",
    )
    en_description = Column(
        "en_description", Text(), nullable=True, comment="期刊、会议或预印本平台的英文简介或说明。"
    )
    establish_time = Column(
        "establish_time", Integer(), nullable=True, comment="期刊首次创办或发行的时间。"
    )
    language = Column(
        "language", String(255), nullable=True, comment="文献或期刊出版使用的语言类型。"
    )
    country = Column(
        "country", String(255), nullable=True, comment="期刊、会议或出版机构所属的国家或地区。"
    )
    eissn = Column(
        "eissn", String(16), nullable=True, comment="期刊电子版的国际标准连续出版物编号。"
    )
    annual_publication = Column(
        "annual_publication", Integer(), nullable=True, comment="期刊或会议每年发表文章的数量。"
    )
    review = Column(
        "review",
        SmallInteger(),
        nullable=True,
        comment="标识该期刊是否为综述类期刊。否=0，是=1，未知=2",
    )
    impact_factor = Column(
        "impact_factor", Numeric(20, 10), nullable=True, comment="期刊或会议的影响力评价指标。"
    )
    jcr_zone = Column(
        "jcr_zone",
        String(2),
        nullable=True,
        comment="期刊在 JCR 等评价体系中的分区信息。Q1,Q2,Q3,Q4",
    )
    open_access = Column(
        "open_access",
        SmallInteger(),
        nullable=True,
        comment="标识期刊或论文是否为开放获取。否=0，是=1，未知=2",
    )
    review_period = Column(
        "review_period", String(255), nullable=True, comment="期刊从投稿到审稿完成的平均周期。"
    )
    scope = Column("scope", String(20), nullable=True, comment="期刊或会议所属的大类学科领域。")
    sub_scope = Column(
        "sub_scope", String(20), nullable=True, comment="期刊或会议所属的细分学科主题。"
    )
    self_rate = Column(
        "self_rate", Numeric(20, 10), nullable=True, comment="期刊文献中自我引用所占的比例。"
    )
    top = Column(
        "top",
        SmallInteger(),
        nullable=True,
        comment="标识期刊或会议是否为所在领域的顶级刊物。否=0，是=1",
    )
    warning = Column(
        "warning",
        SmallInteger(),
        nullable=True,
        comment="标识期刊是否处于预警状态。否=0，是=1，未知=Null",
    )
    is_sci = Column(
        "is_sci",
        SmallInteger(),
        nullable=True,
        comment="标识期刊是否被 SCI 收录。否=0，是=1，未知=Null",
    )
    publish_period = Column(
        "publish_period", String(64), nullable=True, comment="期刊的出版频率或发行周期。"
    )
    jn_official = Column("jn_official", Text(), nullable=True, comment="期刊官方网站地址。")
    layout_cost = Column(
        "layout_cost", String(15), nullable=True, comment="期刊发表论文所需的版面费用。"
    )
    paper_nums = Column(
        "paper_nums", Integer(), nullable=True, comment="期刊、会议或平台已发表论文的数量。"
    )
    updated_time = Column(
...
class DwdEnPaper(Base):
    ...
    publisher = Column(
        "publisher", String(1024), nullable=True, comment="文献或期刊所属的出版商信息。"
    )
    ...
    paper_url = Column(
        "paper_url", Text(), nullable=True, comment="文献在官网或来源平台中的访问链接。"
    )
    doi = Column("doi", String(512), nullable=False, comment="论文的唯一标识编码。")
    cover_date_start = Column(
        "cover_date_start", String(255), nullable=True, comment="文献正式发表的时间。"
    )
...
class DwdEnPaperCitedBy(Base):
    ...
    paper_eid = Column(
        "paper_eid", String(64), nullable=False, comment="被引文献EID（指向dwd_paper_info_qh.eid）"
    )
...
class DwdEnPaperInfo(Base):
    ...
    sdfullavail = Column(
        "sdfullavail", SmallInteger(), nullable=True, comment="全文可获取状态，0-不可获取，1-可获取"
    )
    ...
    absavail = Column(
        "absavail", SmallInteger(), nullable=True, comment="摘要可获取状态，0-不可获取，1-可获取"
    )
    ...
    subj_area = Column(
        "subj_area", String(255), nullable=True, comment="文献所属学科领域，多学科逗号分隔"
    )
    ...
    is_open_access = Column(
        "is_open_access", SmallInteger(), nullable=True, comment="是否开放获取，0-否，1-是"
    )
    oa_article_status = Column(
        "oa_article_status", String(64), nullable=True, comment="开放获取状态描述"
    )
    ...
    author_list = Column(
        ...
        Text(),
        nullable=True,
        comment="作者完整信息列表，多作者分号分隔，含姓名、职称、作者ID等",
    )
    author_surname = Column("author_surname", String(64), nullable=True, comment="第一作者姓氏")
    author_initials = Column(
        "author_initials", String(32), nullable=True, comment="第一作者首字母缩写"
    )
    ...
    source_title_abbrev = Column(
        "source_title_abbrev", String(512), nullable=True, comment="期刊来源简称"
    )
    asjc_code = Column(
        "asjc_code", String(255), nullable=True, comment="ASJC学科分类编码，多编码逗号分隔"
    )
