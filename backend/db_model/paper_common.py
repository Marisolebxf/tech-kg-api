...
class DwdAuthorAffiliation(Base):
    ...
    affiliation_id_parent = Column(
        "affiliation_id_parent", BigInteger(), nullable=True, comment="父机构ID"
    )
...
class DwdAuthorInfo(Base):
    ...
    orcid_matching_type = Column(
        "orcid_matching_type", String(64), nullable=True, comment="ORCID匹配类型"
    )
    ...
    name_variants = Column(
        "name_variants", String(500), nullable=True, comment="姓名别名列表（逗号分隔）"
    )
    history_ids = Column(
        "history_ids", String(500), nullable=True, comment="历史ID列表（逗号分隔）"
    )
    email = Column("email", String(255), nullable=True, comment="邮箱")
    email_type = Column("email_type", String(32), nullable=True, comment="邮箱类型")
    asjc_list = Column(
        "asjc_list", String(500), nullable=True, comment="ASJC学科代码列表，逗号分隔"
    )
    asjc_freq_list = Column(
        "asjc_freq_list", String(500), nullable=True, comment="ASJC频次列表，逗号分隔"
    )
    subjabbr_list = Column(
        "subjabbr_list", String(500), nullable=True, comment="学科缩写列表，逗号分隔"
    )
    subjabbr_freq_list = Column(
        "subjabbr_freq_list", String(500), nullable=True, comment="学科缩写频次，逗号分隔"
    )
    n_affiliation_current = Column(
        "n_affiliation_current", Integer(), nullable=True, comment="当前机构数量"
    )
    current_affiliations = Column(
        "current_affiliations", String(500), nullable=True, comment="当前有效机构ID列表，逗号分隔"
    )
    current_affiliations_parent = Column(
        "current_affiliations_parent", String(500), nullable=True, comment="父机构ID列表，逗号分隔"
    )
    corrupt_xml = Column(
        "corrupt_xml", SmallInteger(), nullable=True, comment="XML文件是否损坏（1=损坏，0=正常）"
    )
