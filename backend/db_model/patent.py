...
class DwdPatent(Base):
    ...
    id = Column(String(36), nullable=False, index=True, comment="行级逻辑主键")
    ...
    application_kind = Column(String(1), comment="专利申请类型代码")
    country_code = Column(String(8), nullable=False, comment="国家、地区或组织代码")
    country = Column(String(20), nullable=False, comment="专利所属或发布国家")
    publication_reference = Column(JSON, comment="发布种类、日期、年份和年月")
    application_reference = Column(JSON, comment="申请号、受理局、日期、年份和年月")
    pct_or_regional_filing_data = Column(JSON, comment="PCT申请号、申请日、进入国家阶段日")
    pct_or_regional_publishing_data = Column(JSON, comment="PCT公布号和公布日期")
    priority_filings = Column(JSON, comment="优先权记录数组")
    applicants = Column(JSON, comment="原始申请人数组")
    assignees = Column(JSON, comment="当前申请人或专利权人数组")
    inventors = Column(JSON, comment="发明人数组")
    ...
    main_classification_ipcr = Column(String(32), comment="IPCR/IPC主分类号")
    further_classification_ipcr = Column(JSON, comment="IPCR/IPC附加分类号")
    main_classification_cpc = Column(String(32), comment="CPC主分类号")
    further_classification_cpc = Column(JSON, comment="CPC附加分类号")
    keywords = Column(JSON, comment="关键词")
    claims = Column(JSON, comment="多语言权利要求")
    description = Column(JSON, comment="多语言说明书")
    figures = Column(JSON, comment="专利附图信息")
    language = Column(JSON, comment="原文语言数组")
    granted_number = Column(String(64), comment="授权号")
    db_source = Column(String(64), comment="来源贴源库")
    create_time = Column(DateTime, comment="创建时间")
    update_time = Column(DateTime, comment="更新时间")
    value = Column(Integer, comment="专利价值")
    agents = Column(JSON, comment="代理人数组")
    agency = Column(JSON, comment="代理机构数组")
    examiners = Column(JSON, comment="审查员数组")
    related_documents = Column(JSON, comment="分案/继续申请信息")
    classification_loc = Column(JSON, comment="LOC分类")
    classification_fi = Column(JSON, comment="FI分类")
    classification_upc = Column(JSON, comment="UPC分类")
    classification_fterm = Column(JSON, comment="F-term分类")


class DwdPatentCited(Base):
    __tablename__ = "dwd_patent_cited"

    id = Column(String(20), nullable=False, index=True)
    patent_id = Column(String(64), primary_key=True)
    reference_cited = Column(Integer, nullable=False)
    cited_by_nums = Column(Integer, nullable=False)
    cited_by_date = Column(JSON)
    patent_citation_date = Column(JSON)
    non_patent_count = Column(Integer, nullable=False)
    non_patent_date = Column(JSON)
    patent_citations_country = Column(JSON)
    patent_citations_region = Column(JSON)
    patent_citations_kd = Column(JSON)
    cited_by = Column(JSON)
    patent_citations = Column(JSON)
    non_patent_citations = Column(JSON)
    db_source = Column(String(64), nullable=False)
...
class DwdPatentTitle(Base):
    ...
    id = Column(String(20), nullable=False, index=True)
    patent_id = Column(String(64), primary_key=True)
    titles = Column(JSON, nullable=False, comment="原文标题")
    title_localized = Column(Text, comment="英文标题")
    title_zh = Column(Text, comment="中文翻译标题")
    db_source = Column(String(64), nullable=False)
    create_time = Column(DateTime, nullable=False)
    update_time = Column(DateTime, nullable=False)
...
class DwdPatentAbstract(Base):
    ...
    id = Column(String(20), nullable=False, index=True)
    patent_id = Column(String(64), primary_key=True)
    abstracts = Column(JSON, nullable=False, comment="原文摘要")
    abstract_localized = Column(Text, comment="英文摘要")
    abstract_zh = Column(Text, comment="中文翻译摘要")
    db_source = Column(String(64), nullable=False)
    create_time = Column(DateTime, nullable=False)
    update_time = Column(DateTime, nullable=False)
...
class DwdPatentLegal(Base):
    ...
    status = Column(String(64), comment="当前法律状态")
    legal_events = Column(JSON, comment="法律事件数组")
    prs_data = Column("patent_legal/prs_data", JSON, comment="PRS法律状态")
    anticipated_expiration = Column(Integer, comment="预计到期日，YYYYMMDD")
    expiration_year = Column(Integer, comment="到期年份")
    db_source = Column(String(64), nullable=False)
    create_time = Column(DateTime, nullable=False)
    update_time = Column(DateTime, nullable=False)


class DwdPatentTransfer(Base):
    __tablename__ = "dwd_patent_transfer"

    id = Column(String(20), nullable=False, index=True)
    patent_id = Column(String(64), primary_key=True)
    country = Column(String(8), nullable=False)
    transfer_effective_date = Column(String(10))
    transfer_before = Column(JSON)
    transfer_after = Column(JSON)
    db_source = Column(String(64), nullable=False)
    create_time = Column(DateTime, nullable=False)
    update_time = Column(DateTime, nullable=False)
...
class DwdPatentFamily(Base):
    ...
    patent_id = Column(String(64), primary_key=True)
    simple_family_number = Column(String(20), nullable=False)
    simple_family_pn = Column(JSON, nullable=False)
    simple_family = Column(JSON, nullable=False)
    family_citations = Column(JSON)
    cited_by_family = Column(JSON)
    patent_family = Column(JSON, nullable=False)
    db_source = Column(String(64), nullable=False)
    create_time = Column(DateTime, nullable=False)
    update_time = Column(DateTime, nullable=False)
