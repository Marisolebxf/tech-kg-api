...
class DwdScholar(Base):
    ...
    scholar_org_name_zh = Column(
        "scholar_org_name_zh", String(1024), nullable=True, comment="中文机构"
    )
    scholar_org_id = Column(
        "scholar_org_id", String(64), nullable=True, comment="所属机构id，学者所属机构id"
    )
    bio = Column(
        "bio", Text(), nullable=True, comment="个人简介/学术简介，学者的个人或学术简介信息"
    )
    bio_zh = Column(
        "bio_zh",
        Text(),
        nullable=True,
        comment="个人简介/学术简介（中文），学者的中文个人或学术简介信息",
    )
    work_experience_en = Column(
        "work_experience_en", Text(), nullable=True, comment="工作经历英文（包含机构和职务）"
    )
    work_experience_zh = Column(
        "work_experience_zh", Text(), nullable=True, comment="工作经历中文（包含机构和职务）"
    )
    education_background_en = Column(
        "education_background_en",
        Text(),
        nullable=True,
        comment="教育背景（英文），学者的英文教育经历信息",
    )
    education_background_zh = Column(
...
class DwdScholarCoauthor(Base):
    ...
    scholar_id = Column(
        "scholar_id", String(32), nullable=False, comment="学者ID，当前学者记录的业务唯一标识"
    )
    co_scholar_id = Column(
        "co_scholar_id",
        String(32),
        nullable=False,
        comment="合作学者ID，合作学者记录的业务唯一标识",
    )
    co_scholar_name_en = Column(
        "co_scholar_name_en", String(256), nullable=True, comment="合作学者英文名"
    )
    co_scholar_name_zh = Column(
        "co_scholar_name_zh", String(128), nullable=True, comment="合作学者中文名"
    )
    co_scholar_avatar = Column(
        "co_scholar_avatar", String(512), nullable=True, comment="合作学者头像URL"
    )
    co_scholar_org_name_en = Column(
        "co_scholar_org_name_en", Text(), nullable=True, comment="合作学者所属机构英文名"
    )
    co_scholar_org_name_zh = Column(
        "co_scholar_org_name_zh", String(1024), nullable=True, comment="合作学者所属机构中文名"
    )
    co_scholar_org_id = Column(
...
class DwdScholarPaperRelation(Base):
    ...
    related_paper_id = Column(
        "related_paper_id", BigInteger(), nullable=True, comment="关联论文库的唯一标识"
    )
