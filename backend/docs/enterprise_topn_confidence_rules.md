...
- 产业链节点：优先展示图节点 `confidence`；图属性缺失时，按核心字段完整度回退计算。权重为 DWD 来源 **0.40**、稳定 `node_id` **0.30**、`node_name` **0.20**、分类/层级等支撑属性 **0.10**。
- 企业、专家：展示图节点自身的 `confidence`，缺失时仍显示“暂无”，不伪造业务值。
- 事件节点（图上的“事件实体”）：展示第 1 节的事件置信度。

`IC0007007` 来自 `dwd_industry_chain_info`，具有稳定 `node_id`、节点名称及层级/阶段/路径等支撑属性，回退置信度为 **1.00**。
