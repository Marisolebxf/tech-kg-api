...
def _strip_quotes(val: Any) -> str:
    ...
    The server may return ``"\"Person\""`` instead of ``"Person"``.
    """
...
class TRSGraphDatabase(GraphDatabase):
    ...
    def get_edge(self, edge_id: Any, edge_type: str | None = None) -> Edge | None:
        # TRS Graph edge ID format: "sourceId->targetId@ranking"
        # The service endpoint requires type param: GET /api/v1/edges/{sourceId}/{targetId}?type=...&ranking=0
        try:
        ...
        # No edge_type provided — look up by scanning source node's edges
        try:
    ...
    def shortest_path(
        ...
        *,
        ...
        # The API may return nodes with only id (empty labels/properties).
        # Fetch full node details for incomplete entries.
    ...
    def batch_create_nodes(
        ...
        resp.raise_for_status()
        data = resp.json()
        # TRS Graph batch API may return empty list even on success;
        # fall back to querying by label to retrieve the created nodes.
        if isinstance(data, list) and data and isinstance(data[0], dict):
            return [_trs_node_to_model(n) for n in data if isinstance(n, dict)]
        # Return empty list — callers can query by label if needed
        return []
    ...
    def batch_create_edges(
        ...
        # TRS Graph expects properties at the top level (flat), not nested under "properties".
        normalized_items = []
        for item in items:
            entry = dict(item)
            ...
            # Flatten nested "properties" dict into top level
    ...
    def list_indexes(self, label: str | None = None) -> list[IndexSpec]:
        ...
        indexes = []
        for item in items:
            # Handle server serialization bug: extra quotes in label/properties
            idx_label = _strip_quotes(item.get("label", ""))
            raw_props = item.get("properties", [])
            # Server may return properties as ["[\"name\"]"] instead of ["name"]
            clean_props = []
            for p in raw_props if isinstance(raw_props, list) else []:
                p = _strip_quotes(p)
                # Server may serialize a list as a string like '["name"]'
