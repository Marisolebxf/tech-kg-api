...
def test_extract_work_experience_zh():
    ...
    resp = client.post(
        "/api/v1/entity/extraction",
        json={"text": "2010年—至今，北京大学计算机科学与技术系，教授", "source_type": "work"},
    )
...
def test_extract_empty_text():
    """空文本应返回 400"""
    resp = client.post("/api/v1/entity/extraction", json={"text": "", "source_type": "work"})
...
def test_extract_invalid_source_type():
    ...
    resp = client.post(
        "/api/v1/entity/extraction",
        json={"text": "2010年—至今，北京大学，教授", "source_type": "invalid_type"},
    )
    assert resp.status_code == 400
...
def test_extract_default_source_type():
    ...
    resp = client.post(
        "/api/v1/entity/extraction", json={"text": "张伟，北京大学教授，研究方向为计算机视觉"}
    )
    assert resp.status_code == 200
