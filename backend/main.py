...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    try:
        # 确保 platform_llm_config / kg_indirect_relation_annotation 表存在
        # （LLM 配置持久化；间接关系人工标注）。
        # MySQL 不可达时跳过建表：CI 无 MySQL 服务，运行期访问相关接口会单独报错。
        ...
        try:
            Base.metadata.create_all(
                get_engine(),
                tables=[LlmConfig.__table__, IndirectRelationAnnotation.__table__],
            )
        except Exception as exc:
            logger.warning(
                "跳过 platform_llm_config/kg_indirect_relation_annotation 建表：MySQL 不可达 %s",
                exc,
            )
