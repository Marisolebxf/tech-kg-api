CREATE TABLE IF NOT EXISTS dwd_org_recruit_info (
    `org_id` VARCHAR(255) NOT NULL COMMENT '机构身份唯一id，用于关联其它表。',
    `name_cn` VARCHAR(255) NOT NULL COMMENT '机构名称',
    `external_id` VARCHAR(255) COMMENT '统一社会信用代码',
    `job_title` VARCHAR(255) COMMENT '岗位',
    `job_description` TEXT COMMENT '工作描述',
    `work_place` TEXT COMMENT '工作地点',
    `release_date` DATE COMMENT '发布日期',
    `hiring_number` BIGINT COMMENT '招聘人数',
    `data_source` VARCHAR(255) NOT NULL COMMENT '数据来源',
    `created_time` DATETIME NOT NULL COMMENT '创建时间',
    `updated_time` DATETIME NOT NULL COMMENT '更新时间',
    KEY `idx_dwd_org_recruit_info_org_id` (`org_id`),
    KEY `idx_dwd_org_recruit_info_external_id` (`external_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='招聘信息';
