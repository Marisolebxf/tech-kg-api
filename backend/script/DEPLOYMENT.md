...
- **`dwd_scholar` 无 `scholar_org_id` 列**：`load_scholar_relations` 的 `AFFILIATED_WITH` 边回退到 `org_{md5(name)[:16]}`，与 `organization_entity_etl` 的 `org_{org_id}`（32位）vid 不一致 → 边悬挂。dev 能跑是因 dev 源表有该列。若需精确 scholar→org 边，用名字 join `dwd_org_base_info.name_cn` 取 org_id 建 `AFFILIATED_WITH`（demo 用例 person_855924f1→新智认知 即如此处理）。
