...
> 单元测试锁定：无 `scholar_org_id` 时按机构名匹配已入图 Organization；匹配不到或机构未入图则跳过，**禁止 md5 桩**。本 smoke 为部署后运行时校验。
