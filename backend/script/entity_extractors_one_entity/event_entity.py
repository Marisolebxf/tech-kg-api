...
Dual-mode 入口：
- CLI: ``python -m script.entity_extractors_one_entity.event_entity --dry-run --limit 1``
...
def build_sources(payload: dict[str, Any]) -> list[tuple[str, str, Any]]:
    ...
    tables = EVENT_TABLES if table_choice == "all" else (table_choice,)
    return [(table, f"SELECT * FROM {table} ORDER BY 1", event_record) for table in tables]
