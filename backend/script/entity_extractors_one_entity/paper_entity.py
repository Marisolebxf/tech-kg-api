"""One-entity extractor for Paper（论文工作流旧口径，支持 --since 增量）。

Dual-mode 入口：
- CLI: ``python -m script.entity_extractors_one_entity.paper_entity --dry-run --limit 1``
...
def build_sources(payload: dict[str, Any]) -> list[tuple[str, str, Any]]:
    ...
    return [(table, f"SELECT * FROM {table} ORDER BY id", paper_record) for table in tables]
