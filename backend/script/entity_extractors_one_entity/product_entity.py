...
Dual-mode 入口：
- CLI: ``python -m script.entity_extractors_one_entity.product_entity --dry-run --limit 1``
...
def build_sources(payload: dict[str, Any]) -> list[tuple[str, str, Any]]:
    ...
    return [(table, f"SELECT * FROM {table} ORDER BY 1", product_record) for table in tables]
