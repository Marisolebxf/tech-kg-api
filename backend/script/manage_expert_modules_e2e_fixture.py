...
NAMES = (
    "陈明远", "李思源", "王海峰", "张若琳", "刘博文", "赵清扬", "周雨辰", "吴静怡",
    "徐志恒", "孙晓彤", "胡嘉伟", "朱雅宁", "高俊杰", "林诗涵", "何宇航", "郭欣然",
    "马致远", "罗婉清", "梁子墨", "宋安琪", "郑凯文", "谢雨桐", "韩东升", "唐梦洁",
    "冯浩然", "于佳宁", "董承泽", "萧语晨", "程瑞阳", "曹芷晴", "袁景行", "邓书瑶",
    "许文昊", "傅心怡", "沈嘉树", "曾可欣", "彭一帆", "吕思琪", "苏景明", "卢晓月",
    "蒋天佑", "蔡依然", "贾正阳", "丁若曦", "魏泽宇", "薛安然", "叶星河", "阎舒雅",
    "余嘉诚", "潘语柔", "杜明哲", "戴欣妍", "夏承宇", "钟灵犀", "汪睿哲", "田可心",
...
def people() -> list[Person]:
    ...
    for count, school_zh, school_en in schools:
        for _ in range(count):
            ...
            rows.append(Person(number, NAMES[number - 1], school_zh, school_en, degree_zh, degree_en, dates[(number - 1) % len(dates)]))
            number += 1
...
def papers() -> list[Paper]:
    rows = [
        ...
        Paper(4, "科研合作网络的演化规律分析", "Evolution of Scientific Collaboration Networks", 2022, (1, 4), ("合作网络",)),
        Paper(5, "知识图谱增量更新关键技术", "Incremental Updating for Knowledge Graphs", 2018, (1, 5), ("增量计算",)),
        Paper(6, "跨语言学术知识融合方法", "Cross-lingual Academic Knowledge Fusion", 2021, (1, 5), ("知识融合",)),
        Paper(7, "亿级图数据并行查询优化", "Parallel Query Optimization for Billion-scale Graphs", 2024, (1, 5), ("图查询", "并行计算"), ("科技创新优秀成果奖",)),
        Paper(8, "弱监督条件下的专家画像构建", "Expert Profiling under Weak Supervision", None, (1, 6), ("专家画像",)),
        Paper(9, "科技成果语义检索模型", "Semantic Retrieval for Scientific Achievements", 2019, (1, 7), ("语义检索",)),
        Paper(10, "可解释科研主题发现算法", "Interpretable Research Topic Discovery", 2025, (1, 8), ("主题发现",)),
...
def scenario_manifest() -> dict[str, list[str]]:
    ...
    return {
        "校友关系": [
            "同校/异校", "同学历/不同学历", "教育日期完全重叠/部分重叠/边界相交/不重叠",
            "中文院校/英文院校", "半角空格/全角空格/研究生院扩展名",
            "仅院校/仅学历日期/教育字段全空", "列表数量超过50", "学校过滤/学历过滤/组合过滤/无结果过滤",
            "有合著边/无合著边", "共同论文/共同项目/共同专利/多类型互动/完全无互动",
        ],
        "两点合作成果": [
            "无共同成果", "仅1篇论文", "多篇论文", "仅专利", "仅项目", "论文+专利+项目",
            "单类型合作", "多类型合作", "长期稳定型合作", "成果获奖统计",
            "开始年份/结束年份/区间过滤", "无法解析或缺失时间", "成果类型过滤", "每类数量限制",
            "同一专家", "源专家不存在", "目标专家不存在",
        ],
    }
...
def plan() -> dict[str, Any]:
    ...
    return {
        ...
        "counts": {
            "persons": len(ps), "papers": len(pas), "projects": len(prs), "patents": len(pts),
...
def _delete_mysql(con) -> None:
    params = {"prefix": PREFIX + "%", "batch": BATCH, "paper_low": PAPER_ID_BASE + 1, "paper_high": PAPER_ID_BASE + 99}
    con.execute(text("DELETE FROM dwd_scholar_coauthor WHERE scholar_id LIKE :prefix OR co_scholar_id LIKE :prefix"), params)
    con.execute(text("DELETE FROM dwd_scholar_paper_relation WHERE scholar_id LIKE :prefix OR paper_id BETWEEN :paper_low AND :paper_high"), params)
    con.execute(text("DELETE FROM dwd_scholar_papers WHERE id BETWEEN :paper_low AND :paper_high"), params)
...
def write_mysql() -> dict[str, int]:
    ...
    try:
        with client.engine.begin() as con:
            _delete_mysql(con)
            con.execute(text("""INSERT INTO dwd_scholar
                ...
                {"sid": p.scholar_id, "name_en": f"E2E Scholar {p.no:03d}", "name_zh": p.name,
                 "bio": f"synthetic fixture; batch={BATCH}", "bio_zh": f"虚构端到端测试数据；批次={BATCH}",
                 "edu_date": p.education_date, "school_en": p.school_en, "degree_en": p.degree_en,
                 "school_zh": p.school_zh, "degree_zh": p.degree_zh, "now": now} for p in people()
            ])
            con.execute(text("""INSERT INTO dwd_scholar_coauthor
                ...
                VALUES (:source,:target,:name_en,:name_zh,'','Future Intelligence Research Center','未来智能研究中心',
                 :count,1,:now,:now)"""), [
                {"source": people()[a - 1].scholar_id, "target": people()[b - 1].scholar_id,
                 "name_en": f"E2E Scholar {b:03d}", "name_zh": people()[b - 1].name, "count": count, "now": now}
                for a, b, count in COAUTHORS
            ])
            con.execute(text("""INSERT INTO dwd_scholar_papers
                ...
                {"id": p.mysql_id, "zh": p.title, "en": p.title_en,
                 "authors": json.dumps([people()[n - 1].scholar_id for n in p.authors], ensure_ascii=False),
                 ...
                 "abstract_en": "Synthetic E2E fixture record.", "doi": f"10.9999/{PREFIX}paper.{p.no}", "now": now}
                for p in papers()
            ])
            paper_relations = [
                {"paper_id": p.mysql_id, "year": p.year or 0, "sid": people()[n - 1].scholar_id,
                 "published": datetime(p.year, 6, 1) if p.year else None, "now": now}
                for p in papers() for n in p.authors
            ]
            con.execute(text("""INSERT INTO dwd_scholar_paper_relation
                (paper_id,year,scholar_id,citations,publish_time,status,create_time,update_time,publication_id,related_paper_id)
                VALUES (:paper_id,:year,:sid,0,:published,1,:now,:now,0,:paper_id)"""), paper_relations)
            con.execute(text("""INSERT INTO dwd_zh_project
                ...
                {"id": p.mysql_id, "number": f"E2E-PROJECT-{p.no:03d}", "title": p.title, "batch": BATCH,
                 "approval_year": p.year, "approval_time": datetime(p.year, 3, 1).date(),
                 "host": people()[p.host - 1].scholar_id,
                 "participants": json.dumps([people()[n - 1].scholar_id for n in p.participants], ensure_ascii=False),
                 "keywords": json.dumps(list(p.fields), ensure_ascii=False), "abstract": f"{p.title}的虚构测试记录",
                 "url": f"https://example.invalid/{BATCH}/project/{p.no}", "now": now} for p in projects()
            ])
            con.execute(text("""INSERT INTO dwd_zh_project_output
                ...
                {"id": p.mysql_id, "total": len(p.awards), "award_count": len(p.awards),
                 "awards": json.dumps(list(p.awards), ensure_ascii=False), "now": now} for p in projects()
            ])
            con.execute(text("""INSERT INTO dwd_patent
                ...
                {"row_id": f"e2e-patent-{p.no:03d}", "patent_id": p.patent_id,
                 "publication_number": f"CN{p.year}E2E{p.no:03d}A",
                 "publication_reference": json.dumps({"year": p.year, "date": f"{p.year}-09-01"}, ensure_ascii=False),
                 "inventors": json.dumps([{"scholar_id": people()[n - 1].scholar_id, "name": people()[n - 1].name} for n in p.inventors], ensure_ascii=False),
                 "first_inventor": people()[p.inventors[0] - 1].name,
                 "keywords": json.dumps(list(p.fields), ensure_ascii=False), "batch": BATCH, "now": now} for p in patents()
            ])
            con.execute(text("""INSERT INTO dwd_patent_title
                ...
                {"row_id": f"e2e-title-{p.no:03d}", "patent_id": p.patent_id,
                 "titles": json.dumps({"zh": p.title, "en": p.title_en}, ensure_ascii=False),
                 "title_en": p.title_en, "title_zh": p.title, "batch": BATCH, "now": now} for p in patents()
            ])
    finally:
        client.dispose()
    return plan()["counts"]
...
def sync_graph_from_mysql() -> dict[str, int]:
    ...
    try:
        with client.engine.connect() as con:
            scholar_rows = con.execute(text("SELECT * FROM dwd_scholar WHERE scholar_id LIKE :p ORDER BY scholar_id"), {"p": PREFIX + "%"}).mappings().all()
            paper_rows = con.execute(text("SELECT * FROM dwd_scholar_papers WHERE id BETWEEN :a AND :b ORDER BY id"), {"a": PAPER_ID_BASE + 1, "b": PAPER_ID_BASE + 99}).mappings().all()
            paper_rel_rows = con.execute(text("SELECT paper_id,scholar_id FROM dwd_scholar_paper_relation WHERE paper_id BETWEEN :a AND :b"), {"a": PAPER_ID_BASE + 1, "b": PAPER_ID_BASE + 99}).mappings().all()
            project_rows = con.execute(text("SELECT * FROM dwd_zh_project WHERE id LIKE :p ORDER BY id"), {"p": PREFIX + "project_%"}).mappings().all()
            project_output_rows = con.execute(text("SELECT id,output_awards FROM dwd_zh_project_output WHERE id LIKE :p"), {"p": PREFIX + "project_%"}).mappings().all()
            patent_rows = con.execute(text("SELECT p.*,t.title_zh,t.title_localized FROM dwd_patent p LEFT JOIN dwd_patent_title t ON t.patent_id=p.patent_id WHERE p.patent_id LIKE :p ORDER BY p.patent_id"), {"p": PREFIX + "patent_%"}).mappings().all()
            coauthor_rows = con.execute(text("SELECT scholar_id,co_scholar_id,co_paper_count FROM dwd_scholar_coauthor WHERE scholar_id LIKE :p"), {"p": PREFIX + "%"}).mappings().all()
    finally:
        client.dispose()
    ...
    for fixture_vid in [*(p.vid for p in people()), *(p.vid for p in papers()), *(p.vid for p in projects()), *(p.vid for p in patents())]:
        graph.delete_node(fixture_vid, detach=True)

    def merge_edge(source: str, target: str, edge_type: str, key: str, props: dict[str, Any] | None = None) -> None:
        graph.create_edge(source, target, edge_type, props or {})

    try:
        for row in scholar_rows:
            sid = row["scholar_id"]
            graph.merge_node(["Person"], {"vid": f"person_{sid}"}, {
                "name_zh": row["name_zh"] or "", "name_en": row["name_en"] or "",
                "scholar_org": row["scholar_org_name_zh"] or row["scholar_org_name_en"] or "",
                "biography": row["bio"] or "", "bio_zh": row["bio_zh"] or "",
                "education_background_date": row["education_background_date"] or "",
                "education_background_institution_en": row["education_background_institution_en"] or "",
                "education_background_degree_en": row["education_background_degree_en"] or "",
                "education_background_institution_zh": row["education_background_institution_zh"] or "",
                "education_background_degree_zh": row["education_background_degree_zh"] or "",
                "source_system": "gkx_element", "source_table": "dwd_scholar", "source_record_id": sid,
                "ingest_batch": BATCH, "ingest_time": now, "scholar_status": int(row["status"] or 0),
            })
        paper_defs = {p.mysql_id: p for p in papers()}
        for row in paper_rows:
            definition = paper_defs[row["id"]]
            graph.merge_node(["Paper"], {"vid": definition.vid}, {
                "title_zh": row["zh_name"], "title_en": row["en_name"],
                "publication_year": str(definition.year or ""),
                "publication_date": row["cover_date_start"].strftime("%F") if row["cover_date_start"] else "",
                "doi": row["doi"], "source": BATCH,
            })
        for row in paper_rel_rows:
            merge_edge(f"paper_{row['paper_id']}", f"person_{row['scholar_id']}", "AUTHORED_BY", f"paper:{row['paper_id']}:author:{row['scholar_id']}")
        for row in project_rows:
            pvid = f"project_{row['id']}"
            graph.merge_node(["Project"], {"vid": pvid}, {
                "title": row["title"], "approval_year": str(row["approval_year"] or ""),
                "abstract": row["abstract"] or "", "awards_count": 1 if output_awards.get(row["id"]) not in (None, "", "[]") else 0,
                "source_system": "gkx_element", "source_table": "dwd_zh_project", "source_record_id": row["id"],
                "ingest_batch": BATCH, "ingest_time": now,
            })
            ...
            for sid in json.loads(row["participants"] or "[]"):
                merge_edge(pvid, f"person_{sid}", "HAS_PARTICIPANT", f"project:{row['id']}:participant:{sid}")
        for row in patent_rows:
            pvid = f"patent_{row['patent_id']}"
            publication = row["publication_reference"] if isinstance(row["publication_reference"], dict) else json.loads(row["publication_reference"] or "{}")
            keywords = row["keywords"] if isinstance(row["keywords"], list) else json.loads(row["keywords"] or "[]")
            graph.merge_node(["Patent"], {"vid": pvid}, {
                "title_zh": row["title_zh"] or "", "title_en": row["title_localized"] or "",
                "publication_date": int(str(publication.get("date") or "").replace("-", "") or 0), "keywords": json.dumps(keywords, ensure_ascii=False),
                "publication_number": row["publication_number"], "patent_id": row["patent_id"],
                "db_source": BATCH,
            })
            inventors = row["inventors"] if isinstance(row["inventors"], list) else json.loads(row["inventors"] or "[]")
            for inventor in inventors:
                sid = inventor.get("scholar_id")
                if sid:
                    merge_edge(pvid, f"person_{sid}", "INVENTED_BY", f"patent:{row['patent_id']}:inventor:{sid}")
        for row in coauthor_rows:
            merge_edge(f"person_{row['scholar_id']}", f"person_{row['co_scholar_id']}", "COAUTHOR_WITH", f"coauthor:{row['scholar_id']}:{row['co_scholar_id']}", {"co_paper_count": int(row["co_paper_count"] or 0)})
        for query in (
...
def verify() -> dict[str, Any]:
    ...
    with client.engine.connect() as con:
        mysql_counts = {
            "persons": con.execute(text("SELECT COUNT(*) FROM dwd_scholar WHERE scholar_id LIKE :p"), {"p": PREFIX + "%"}).scalar_one(),
            "papers": con.execute(text("SELECT COUNT(*) FROM dwd_scholar_papers WHERE id BETWEEN :a AND :b"), {"a": PAPER_ID_BASE + 1, "b": PAPER_ID_BASE + 99}).scalar_one(),
            "projects": con.execute(text("SELECT COUNT(*) FROM dwd_zh_project WHERE id LIKE :p"), {"p": PREFIX + "project_%"}).scalar_one(),
            "patents": con.execute(text("SELECT COUNT(*) FROM dwd_patent WHERE patent_id LIKE :p"), {"p": PREFIX + "patent_%"}).scalar_one(),
        }
        encoding_errors = con.execute(text("SELECT COUNT(*) FROM dwd_scholar WHERE scholar_id LIKE :p AND (name_zh LIKE '%æ%' OR name_zh LIKE '%�%' OR education_background_institution_zh LIKE '%æ%')"), {"p": PREFIX + "%"}).scalar_one()
    client.dispose()
    graph = get_trs_graph_client()
    try:
        graph_nodes = {
        ...
        for p in people():
            ...
            if props.get("name_zh", "") != p.name or props.get("education_background_institution_zh", "") != (p.school_zh or ""):
                field_mismatches.append(p.scholar_id)
    finally:
        close_trs_graph_client()
    ok = mysql_counts == {k: expected[k] for k in mysql_counts} and graph_nodes == mysql_counts and encoding_errors == 0 and not field_mismatches
    return {"ok": ok, "batch": BATCH, "mysql": mysql_counts, "graph": graph_nodes, "encodingErrors": encoding_errors, "fieldMismatches": field_mismatches, "scenarioManifest": scenario_manifest()}
...
def cleanup() -> dict[str, Any]:
    ...
    graph = get_trs_graph_client()
    try:
        for vid in [*(p.vid for p in people()), *(p.vid for p in papers()), *(p.vid for p in projects()), *(p.vid for p in patents())]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="两个专家模块的 MySQL -> dev 图空间端到端测试数据闭环")
    action = parser.add_mutually_exclusive_group()
    action.add_argument("--apply", action="store_true", help="幂等重建 MySQL 数据并从 MySQL 回读同步到 dev")
