"""One-relation extractor: INVESTS_IN（Organization → Organization）.

复刻旧 organization_relation_etl.py 口径：投资事件表，边属性投资金额/比例。
确定性 rank 幂等，端点验存，虚拟源行过滤；关系脚本一律不建顶点。

Dual-mode 入口：
- CLI: ``python -m script.relation_extractors_one_relation.invests_in_relation --dry-run --limit 1``
