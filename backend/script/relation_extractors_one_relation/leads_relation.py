"""One-relation extractor: LEADS（Project → Person）.
...
Dual-mode 入口：
- CLI: ``python -m script.relation_extractors_one_relation.leads_relation --dry-run --limit 1``
...
def collect_person_candidates(
    ...
    *,
    ...
    """旧 collect_match_candidates 的 person 通道：project_host 原值 + participants 解析值。"""
...
def _collect_candidates(
    ...
    """连 MySQL 收集 person 候选（旧 main 里 try/finally dispose 那段）。"""
...
def _load_matcher(candidates: set[str], dry_run: bool) -> tuple[ProjectEntityMatcher, bool]:
    ...
    try:
        ...
        if not dry_run:
            ensure_edge_schema(graph, "LEADS", EDGE_SCHEMA)
    finally:
        graph.close()
    return matcher
...
def main() -> None:
    ...
    sources = [
        (table, PROJECT_SQL.format(table=table), make_leads_mapper(matcher, report))
        for table in tables
    ]
