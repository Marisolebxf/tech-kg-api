...
class TestTRSGraphDatabaseConnect:
    ...
    @patch("graph_db.backends.trs_graph_backend.httpx.Client")
    def test_connect_with_api_key(self, mock_client_cls):
        mock_client = MagicMock()
        mock_client.get.return_value = _make_mock_response(200, {"status": "UP"})
        mock_client_cls.return_value = mock_client

        db = TRSGraphDatabase("http://localhost:8090", "test_space", api_key="ysukeg")
        db.connect()

        mock_client_cls.assert_called_once_with(
            base_url="http://localhost:8090",
            headers={"X-Graph-Space": "test_space", "X-API-Key": "ysukeg"},
            timeout=30,
        )

    @patch("graph_db.backends.trs_graph_backend.httpx.Client")
    def test_connect_without_api_key_no_header(self, mock_client_cls):
        mock_client = MagicMock()
        mock_client.get.return_value = _make_mock_response(200, {"status": "UP"})
        mock_client_cls.return_value = mock_client
...
class TestSchemaManagement:
    ...
    def test_list_indexes(self, mock_client_cls):
        # REST API returns a list of index objects
        index_data = [
        ...
        assert indexes[1].label == "Company"

    @patch("graph_db.backends.trs_graph_backend.httpx.Client")
    def test_list_indexes_with_quoted_labels(self, mock_client_cls):
        # Server bug: may return extra quotes like "\"Person\""
        index_data = [
            {"label": '"Person"', "properties": ['["name"]'], "unique": False},
        ]
        db, mock_client = self._setup_db(mock_client_cls, index_data)
    ...
    @patch("graph_db.backends.trs_graph_backend.httpx.Client")
    def test_create_constraint(self, mock_client_cls):
        from graph_db.models import ConstraintSpec

        db, mock_client = self._setup_db(mock_client_cls)
        spec = ConstraintSpec(name="person_name_uq", label="Person", property="name", kind="unique")
        db.create_constraint(spec)
    ...
    def test_list_constraints(self, mock_client_cls):
        ...
        assert constraints[0].name == "person_name_uq"
        assert constraints[0].label == "Person"
        assert constraints[0].property == "name"
