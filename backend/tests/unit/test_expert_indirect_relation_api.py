...
def test_builds_only_indirect_paths_and_deduplicates_reverse_edges():
    ...
    assert result["indirectNodeCount"] == 2
    assert result["pathCount"] == 2
    assert result["relationTypeCount"] == {"项目关联": 1, "学术关联": 1}
    assert {path["targetNode"]["name"] for path in result["paths"]} == {
        "专家丙",
        "机构一",
    }
...
def test_relation_type_filter_and_string_normalization():
    ...
    assert body.relation_types == ["学术关联", "机构关联"]
