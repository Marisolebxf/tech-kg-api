...
def test_source_fallback_verifies_existing_node():
    ...
    graph = MagicMock()
    graph.get_node.return_value = SimpleNamespace(
        labels=["Organization"], properties={"name_cn": "甲公司"}
    )
...
def test_targeted_affiliations_use_requested_scholars():
    ...
    with patch(
        "script.load_scholar_relations._iter_scholar_affiliations", return_value=[]
    ) as records:
        assert (
            load_affiliations(None, MagicMock(), dry_run=True, scholar_ids=["abc"])["written"] == 0
        )
