...
- 上传 `.py` 脚本：大小上限（`max_script_bytes`）、UTF-8（含 BOM 容忍）、`ast.parse` 语法检查、**必须有 `transform(payload)` 入口**（旧 `workflow(payload)` 兼容），或用 **`@step` 装饰器**（`from kg_sdk import step` 后在顶层函数上标注 `@step` / `@step("id")`；步顺序 = 函数源码出现顺序，id 默认取函数名，匹配 `[A-Za-z0-9_-]{1,64}` 且唯一、与 `transform` 互斥、最长 16 步），或兼容旧顶层 **`STEPS` 多步清单**（`[{"id": ..., "fn": ...}, ...]` list 字面量，fn 必须是顶层函数）。两种多步声明不能同时使用，`service/script_steps.py` 上传时纯 AST 静态校验尽早报错——脚本只做"接收行数据 → 输出 `{entities|edges, failures}` JSON"的纯转换。
...
- **脚本约定**：`transform(payload)` 只输出 `{entities|edges, failures}`；多步脚本推荐用 `@step` 装饰器标注顶层函数（`from kg_sdk import step`，顺序 = 源码顺序；兼容旧顶层 `STEPS` 清单；均与 `transform` 互斥），每步同契约、额外键流向下一步 `input`（示例见 `docs/抽取脚本示例/多步示例_三步出边.py`）；平台负责读源/写图/消歧/索引/游标——脚本零副作用，安全边界靠 LLM 审计 + 入口检查兜底。
...
sequenceDiagram
    ...
    SVC->>SVC: ast.parse + 入口检查（优先 transform(payload)，兼容 workflow(payload)；或 @step 装饰器 / 顶层 STEPS 多步清单）
...
sequenceDiagram
    ...
    loop 每来源 × 分批
        ...
        WK->>WK: 脚本 transform(payload) → {entities|edges, failures}（只做转换）<br/>（多步脚本：按 @step 声明顺序逐步 execute_transform，第 N>1 步 payload.input=上一步输出）
