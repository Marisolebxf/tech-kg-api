...
> 本文保留旧二角色方案及门户身份来源说明。新的业务 `client_id`、三角色、空间审批与生产审核权限以 [业务与图空间权限交付说明](kgetl-business-rbac.md) 为准；该方案由 `BUSINESS_RBAC_ENABLED` 控制。新版部署前即须补齐审核空间列，不能因开关关闭省略数据库迁移。本文旧个人空间绑定、菜单边界不得作为新模式的授权旁路。
