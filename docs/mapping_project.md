...
> 图空间：TRSGraph **`dev`**  
> 源库默认：`gkx_element` 的 `dwd_*_project*`

## 0. 责任表（`gkx_element`）

| 表名 | 说明 |
|------|------|
| `dwd_zh_project` | 国内项目主表 |
| `dwd_en_project` | 国外项目主表 |
| `dwd_zh_project_output` | 国内项目产出 |
| `dwd_en_project_output` | 国外项目产出 |
| `dwd_rel_project_paper` | 有表则建 `OUTPUT_OF`；可能不存在 → skip |
| `dwd_rel_project_patent` | 有表则建 `OUTPUT_OF`；可能不存在 → skip |

**约定：** `output.id = project.id`（1:1）。无 rel 表时产出边来自 output JSON。
...
| 图属性 | 取值 |
|--------|------|
| `source_system` | 固定 `gkx_element`（要素库入图） |
| `source_table` | 实际表名，如 `dwd_zh_project` |
...
## 2. `dwd_zh_project` → Tag `Project` + 边
...
## 3. `dwd_en_project` → Tag `Project` + 边
...
## 4. `dwd_zh_project_output` / `dwd_en_project_output`

> **关联约定**：主键 `id` **等于** 对应项目表 `id`（按 `gkx_element` 约定验证）。映射文档与假数据均按此约定；若源库未来变更需增加显式 `project_id`。
...
| `DataSource` | `source_table`, `table_cn_name`, `tier=element`, `library=gkx_element` | — |
...
| 步骤 | 命令 |
|------|------|
| 灌假数据 | `docker exec -i mysql mysql -uroot -p... gkx_element < backend/schemas/seed/project_fake_data.sql` |
