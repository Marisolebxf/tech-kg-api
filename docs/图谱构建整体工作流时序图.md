...
| SQLite（`backend/var/tech-kg-workflows.db`） | 控制面状态：工作流定义、任务（task）、执行记录（execution）、校正同步任务 |
| MinIO (S3) | 抽取脚本文件存储（`schemas/{kind}/{schema_id}/...`） |
...
4. **轮询刷新**：`GET /workflow-system/executions/{id}` 与 `GET /task-center/tasks/{id}` 在任务 RUNNING 时会主动向 Temporal 拉取最新状态并回写 SQLite，再返回给前端。
...
2. **添加脚本**：上传不含 `workflow(payload)` 的脚本应在 syntax 阶段失败；LLM 未配置 `LLM_API_KEY` 时 llm 阶段报错；成功后 MinIO 对象与 SQLite 定义（`kg.custom.python`）应同时存在。
