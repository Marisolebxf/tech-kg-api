...
> 3. "用户工作流组合已注册算子"**不成立**——工作流 step 从不引用算子名；算子注册表已于 2026-09-14（D4）整体下线（commit `3b20eb6`）。
...
| `kg.schema.extract.chain` | extract | `[{schemaId, schemaKey, name, label}, …]`（顺序即执行序） | **多脚本串行**（2026-09-17 恢复，新形态）：`SchemaExtractChainWorkflow` 子类复用单 Schema 全链路，按 steps 顺序**严格串行**逐个跑 `kg.schema.extract`，任一环失败整链 FAILED（后续环不再执行）；每环独立水位（`schema-extract-{schemaKey}`）/索引/T_EXTRACT_FAIL 语义不变；失败恢复走 Temporal reset 重试。拒绝 `recordIdsBySource`（失败记录重跑仅支持单 Schema 任务）。 |

> 2026-09-14 D2/D3 收敛：`kg.custom.python` / `kg.custom.steps` / `kg.custom.chain`（独立脚本上传通道）与 `kg.entity.*` / `kg.relation.*` / `kg.graph.build`（builtin 域 stub 族）均已删除——用户代码唯一入口是 Schema 管理上传，唯一执行引擎是 `kg.schema.extract`；`execute_kg_step` 空壳 activity 一并移除（configurable 改为内联记账）。2026-09-17 chain 以新形态恢复（串对象从"上传脚本"改为"Schema 抽取"，见上表），旧 `kg.custom.chain` 定义不复活——存量旧链 job 触发会收到中文报错引导重建。
...
- **Job**（任务中心的用户视图，taskType：`extract`（单 Schema）/ `chain`（多脚本串行，`schemaIds` 2–20 个）——D2 后 single/upload 停止新建，存量行触发会因 workflow 未注册而失败；存量旧 chain 行（定义 workflowType 非 `kg.schema.extract.chain`）触发报错引导重建）：cron Job 自动建 `{job_id}-sched` Schedule；启停 Job 同步 pause/resume Schedule。
...
- **批载荷 S3 中转（claim-check，`SCHEMA_EXTRACT_PAYLOAD_S3_ENABLED` 默认关）**：开启后批行/转换输出/步间透传/失败清单归档 RustFS S3（`service/extract_payload_store.py`，key 布局 `runs/{workflowId}/{runId}/payloads/{stepId}/{batchIdx}/{artifact}.json.gz`，gzip、activity 重试同 key 覆盖幂等），事件历史只进元数据（key + 计数），脚本契约不变（activity 内下载后组装的 payload 与内联路径逐字节等价）。批预算随之中转开 = 8MB（`SCHEMA_EXTRACT_PAYLOAD_MAX_BYTES`，大文本行批吞吐恢复），关 = 512KB（历史行为）。留档默认 7 天（`SCHEMA_EXTRACT_PAYLOAD_RETENTION_DAYS`），worker 每日扫描清理（`SCHEMA_EXTRACT_PAYLOAD_CLEANUP_ENABLED`）并幂等套用 bucket lifecycle（prefix=runs/）。兼容策略：workflow 按「已记录进历史的结果形状」分支（`chunks`/`outKey`/`recordsKey`/`failuresKey` 在则走中转），在飞旧 run 重放安全，无需排空即可翻 flag。
...
│                                               #   chain 任务每个 Schema 是可展开抽屉，内层为脚本转换步）
├── components/JobLaunchDialog.vue              # 新建 Job（extract：选 schema + 批大小；
│                                               #   chain：有序 Schema 队列 ↑/↓/×，≥2 个才可提交）
...
| `POST /jobs` | 创建 Job（taskType extract=单 Schema / chain=schemaIds 串行 2–20 个；其余历史值拒绝） |
...
| `workflow_jobs` | 任务中心 Job | id、name(idx)、task_type(idx)（现新增 extract/chain；存量 single/upload 及旧 chain 待清理——`chain-` 前缀的定义 id 新旧共用，清理须按定义 workflowType 判别，不可按前缀一刀切）、definition_id、owner(idx)、status（启用/暂停）、schedule_kind(once/cron)、cron、payload（chain 任务另带 schemaIds/schemaLabels） |
...
sequenceDiagram
    ...
    par 来源间并行（asyncio.gather）
        loop 每来源：1 reader 串行读 + N worker（Queue 背压，并发 ≤ SCHEMA_EXTRACT_MAX_INFLIGHT）
            WK->>RES: read_source_batch activity（水位/pk keyset 或 LIMIT/OFFSET；预算超限折半）<br/>S3 中转开：批行归档 runs/ 下，返回 chunks 元数据（8MB 载荷预算）
            WK->>S3: execute_transform activity 内按需下载 transform 脚本（+ rowsKey/inputKey 载荷）
            WK->>SUB: _spawn_script(_SINGLE_ARG_RUNNER, transform(payload))
            note over WK,SUB: KG_SCRIPT_CTX env 注入连接参数（密钥不进 payload）；<br/>KG_ACCESS_LOG 溯源 sidecar；脚本只输出 {entities|edges, failures}；<br/>逐行失败 → T_EXTRACT_FAIL 审核 case<br/>多步脚本（@step 装饰器声明，兼容顶层 STEPS 清单）：按步多次 execute_transform（各自独立重试），<br/>第 N>1 步 payload.input=上一步完整输出、ctx.prev_outputs 可读已完成各步输出
...
> 旧 `kg.custom.steps`（多步流水线上传通道）与 `kg.custom.python`（单脚本上传）的逐步执行图已随 D2 删除——两通道的时序与上图子进程 runner 机制一致，仅 steps 数组来源不同。多步能力已于 2026-09-15 并入 `kg.schema.extract` 主通道：用户在 Schema 脚本里用 `@step` 装饰器标注顶层函数（推荐；顺序 = 源码出现顺序，兼容旧顶层 `STEPS` 清单；不再有独立上传通道/双参 runner），平台按序逐步执行 `execute_transform`，语义详见 `docs/Schema管理模块.md` 第 4 节。
>
> **chain（`kg.schema.extract.chain`，2026-09-17 恢复）**：payload 为扁平 `schemaIds` 列表（无 `{definitionId, payload}` 包装），`run()` 按 steps 顺序对每个 Schema 完整走一遍上图流程（读源→transform→写图→推水位→索引），任一环 FAILED 即整链 FAILED 且后续环不执行。进度经 `@query get_steps` 实时暴露（键 `schema:{schemaId}`），终态输出落库 `output.steps`（含每环 `records/written/failed` 与内层 `activities` = 脚本各转换步聚合；单 `transform` 脚本聚合为 `_default` 一条）；任务详情页据此渲染"每个 Schema 一个抽屉、抽屉内是转换步"的两层视图。
