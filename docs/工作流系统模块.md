...
sequenceDiagram
    ...
    par 来源间并行（asyncio.gather）
        loop 每来源：1 reader 串行读 + N worker（Queue 背压，并发 ≤ SCHEMA_EXTRACT_MAX_INFLIGHT）
            ...
            note over WK,SUB: KG_SCRIPT_CTX env 注入连接参数（密钥不进 payload）；<br/>KG_ACCESS_LOG 溯源 sidecar；脚本只输出 {entities|edges, failures}；<br/>逐行失败 → T_EXTRACT_FAIL 审核 case
...
> 旧 `kg.custom.steps`（多步流水线上传通道）与 `kg.custom.python`（单脚本上传）的逐步执行图已随 D2 删除——两通道的时序与上图子进程 runner 机制一致，仅 steps 数组来源不同。
