...
演示在脚本顶层声明 ``STEPS`` 清单的写法（kg.schema.extract 主通道内建多步能力，
不需要独立上传通道）。场景：源表每行含 scholar_id / raw_org_name / position /
start_date，产出 EMPLOYED_BY 边。

与单 ``transform`` 脚本同构的约定：

- 每步是单参 ``step_fn(payload)`` 纯转换；平台按 STEPS 顺序执行，每步一次
  Temporal activity（第 k 步失败只重试第 k 步，前序步输出经事件历史重放）；
...
# 平台按序执行：第 1 步消费平台读的源表行，后续步消费上一步输出
STEPS = [
...
def step_normalize(payload: dict[str, Any]) -> dict[str, Any]:
...
def step_resolve(payload: dict[str, Any]) -> dict[str, Any]:
...
def step_emit(payload: dict[str, Any]) -> dict[str, Any]:
