...
> **⚠️ 本模块已于 2026-09-14 整体下线（清理批次 4-D4）**：`service/operator_registry.py`、`service/operator_builtins.py`、`infra/operator_store.py`、`biz/handler/operator.py`、`application/operator.py`、`operators/` 目录、`script/register_scholar_operators.py` 及 `/api/v1/operators` 全部端点均已删除；`OPERATOR_*` env 与 `tech-kg-operators` bucket（RustFS 实例保留，归档后清空该 bucket）一并退场。学者域的 5 个"算子"本就是 `script/` 下 ETL 的薄包装——能力本体（`load_scholar_*`、`align_scholar_affiliations.py`、`dedupe_scholar_persons.py`、`build_*_milvus_index.py`，由 `rebuild_scholar_graph.py` 编排）全部保留（D5 暂缓删除）。用户代码唯一入口收敛到 Schema 管理上传，唯一执行引擎是 `kg.schema.extract`（见 `docs/无用代码清理与合并清单.md` 第五节）。本文保留下文仅作历史设计记录。
