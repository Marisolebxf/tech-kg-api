...
基础设施工件（Milvus + etcd、共用一个 RustFS S3 承载 schema 脚本 / Milvus 内部存储、m3e-embedding、auth-redis、Temporal）都在生产 compose 里。Milvus 用专用端口避开宿主 `tech-kg-engine` 的 Milvus：SDK `19531`、健康 `9093`；RustFS S3 宿主端口 `9020` / 控制台 `9021`。
