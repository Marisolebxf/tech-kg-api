...
import { Checkbox as ACheckbox, Form as AForm, FormItem as AFormItem, Option as AOption, Select as ASelect, Textarea as ATextarea } from '@arco-design/web-vue'
...
const toggleEvidence=(item:unknown,checked:boolean)=>{ value.evidence=checked?[...value.evidence,item]:value.evidence.filter((entry)=>entry!==item) }
</script>
<template>
  <a-form :model="value" class="dynamic-form" layout="vertical">
    <section v-for="(section,index) in sections" :key="`${section.type}-${index}`" class="dynamic-section">
      <template v-if="section.type==='mapping-table'">
        <h3>字段/字典映射</h3><a-form-item field="mappingsJson"><a-textarea v-model="value.mappingsJson" :auto-size="{ minRows: 4, maxRows: 10 }" placeholder='[{"source":"源字段","target":"目标字段"}]' /></a-form-item>
      </template>
      ...
      <template v-else-if="section.type==='attribute-comparison'">
        <h3>属性来源对照</h3><pre>{{ json(candidate?.conflicts) }}</pre><a-form-item field="fieldsJson"><a-textarea v-model="value.fieldsJson" :auto-size="{ minRows: 4, maxRows: 10 }" placeholder='{"属性":"最终值"}' /></a-form-item>
      </template>
      <template v-else-if="section.type==='runtime-config'">
        <h3>运行配置</h3><pre>{{ json(candidate?.runtime) }}</pre><a-form-item field="runtimeJson"><a-textarea v-model="value.runtimeJson" :auto-size="{ minRows: 4, maxRows: 10 }" placeholder='{"model":"...","timeoutSeconds":60}' /></a-form-item>
      </template>
