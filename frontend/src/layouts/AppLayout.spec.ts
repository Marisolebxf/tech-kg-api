...
const sharedPaths = [
  '/graph-query', '/graph-query/entities', '/expert-direct', '/node-indirect', '/two-point-achievement',
  '/expert-colleague', '/expert-alumni', '/paper-cooperation', '/enterprise-relation',
  '/industry-chain-event', '/industry-chain-panorama',
]
const managementPaths = ['/overview', '/schema', '/graph-build', '/manual-review', '/configurations']
...
async function renderLayout(isAdmin: boolean, path = '/graph-query', collapsed = false) {
...
describe('既有侧边栏按有效管理员身份显隐', () => {
  it('普通用户只见查询与服务，展开时所有共享子菜单保留', async () => {
    ...
    expect(navigation.text()).toContain('查询与服务')
  ...
  it('管理员继续看到全部原有分组及管理端入口', async () => {
    const { wrapper } = await renderLayout(true)
    const navigation = wrapper.get('.app-nav')
    for (const name of ['工作台', '图谱建设与治理', '平台管理', '查询与服务']) {
  ...
  it('侧边栏收起后普通用户不能通过图标或飞出菜单进入管理页', async () => {
    const { wrapper } = await renderLayout(false, '/graph-query', true)
    const navigation = wrapper.get('.app-nav')
    for (const path of managementPaths) expect(navigation.find(`a[href="${path}"]`).exists()).toBe(false)
    for (const path of sharedPaths) expect(navigation.find(`a[href="${path}"]`).exists()).toBe(true)
  })
  ...
  it('有效角色刷新为普通用户后现有管理菜单立即隐藏', async () => {
    const { wrapper, auth } = await renderLayout(true)
    auth.profile!.isAdmin = false
    await nextTick()
    expect(wrapper.find('.app-nav a[href="/schema"]').exists()).toBe(false)
    expect(wrapper.find('.app-nav a[href="/graph-query"]').exists()).toBe(true)
  })
