...
import { fetchPaperStructuredResult } from '../../api/paper-cooperation'
...
const defaultNodeLayout: Record<NodeId, NodeLayout> = {
  expertA: { x: 3, y: 2, w: 27, h: 13 },
  expertB: { x: 70, y: 2, w: 27, h: 13 },
  paper: { x: 28, y: 21, w: 44, h: 21 },
  topic: { x: 2, y: 49, w: 34, h: 21 },
  period: { x: 36, y: 81, w: 24, h: 10 },
  contribution: { x: 58, y: 45, w: 38, h: 32 },
}
...
const graphInteraction = useGraphNodeInteraction({
  coordMode: 'percent',
  getBoardElement: () => boardRef.value ?? previewViewportRef.value,
  getCanvasSize: () => ({ width: 100, height: 100 }),
  getNodeSize: (id) => {
    const node = previewNodes.value[id as NodeId]
...
const previewGraph = computed(() => {
  ...
  return {
    ...
    relationPill: midpoint(topStart, topEnd, 0, -2.5),
  }
})
...
function syncContributionHeight() {
  const tagRows = Math.ceil(sharedContributionTags.value.length / 2)
  const minH = 26 + tagRows * 5 + 12
  previewNodes.value = {
    ...previewNodes.value,
    contribution: {
      ...previewNodes.value.contribution,
      h: Math.min(40, Math.max(30, minH)),
    },
  }
}

function resolveNodeOverlaps() {
  syncContributionHeight()
  const boxes = (Object.keys(previewNodes.value) as NodeId[]).map((id) => ({
    key: id,
    x: previewNodes.value[id].x,
    y: previewNodes.value[id].y,
    w: previewNodes.value[id].w,
    h: previewNodes.value[id].h,
  }))
  separatePercentNodes(boxes, 2.8, 120)
  const next = { ...previewNodes.value }
  for (const box of boxes) {
    const id = box.key as NodeId
    next[id] = { ...next[id], x: box.x, y: box.y }
  }
  previewNodes.value = next
}
...
async function runTest(payload: TestRequestPayload = appliedParams.value) {
  ...
  error.value = ''
  try {
    apiExample.value = await fetchPaperStructuredResult(payload)
                  ...
                  <div ref="previewViewportRef" class="graph-preview-viewport">
                    <div ref="boardRef" class="graph-preview-board">
                      ...
                      <article
                        ...
                        :class="graphInteraction.nodeClasses('expertA')"
                        :style="nodeStyle('expertA')"
                        @mousedown="graphInteraction.startNodeDrag('expertA', $event, previewNodes.expertA.x, previewNodes.expertA.y)"
                        @click="graphInteraction.handleNodeClick('expertA')"
                      >
                      ...
                      <article
                        ...
                        :class="['node-b expert-card', graphInteraction.nodeClasses('expertB')]"
                        :style="nodeStyle('expertB')"
                        @mousedown="graphInteraction.startNodeDrag('expertB', $event, previewNodes.expertB.x, previewNodes.expertB.y)"
                        @click="graphInteraction.handleNodeClick('expertB')"
                      >
                      ...
                      <article
                        ...
                        :class="graphInteraction.nodeClasses('paper')"
                        :style="nodeStyle('paper')"
                        @mousedown="graphInteraction.startNodeDrag('paper', $event, previewNodes.paper.x, previewNodes.paper.y)"
                        @click="graphInteraction.handleNodeClick('paper')"
                      >
                      ...
                      <article
                        ...
                        :class="graphInteraction.nodeClasses('topic')"
                        :style="nodeStyle('topic')"
                        @mousedown="graphInteraction.startNodeDrag('topic', $event, previewNodes.topic.x, previewNodes.topic.y)"
                        @click="graphInteraction.handleNodeClick('topic')"
                      >
                      ...
                      <article
                        ...
                        :class="graphInteraction.nodeClasses('period')"
                        :style="nodeStyle('period')"
                        @mousedown="graphInteraction.startNodeDrag('period', $event, previewNodes.period.x, previewNodes.period.y)"
                        @click="graphInteraction.handleNodeClick('period')"
                      >
                        <div class="summary-card-body">
                      ...
                      <article
                        ...
                        :class="graphInteraction.nodeClasses('contribution')"
                        :style="nodeStyle('contribution')"
                        @mousedown="graphInteraction.startNodeDrag('contribution', $event, previewNodes.contribution.x, previewNodes.contribution.y)"
                        @click="graphInteraction.handleNodeClick('contribution')"
                      >
            ...
            <section class="developer-code-shell">
              ...
              <div class="developer-code-block">
                <div class="developer-code-scroll">
                  <HighlightedCodeBlock :lines="currentDeveloperCodeLines" />
                </div>
