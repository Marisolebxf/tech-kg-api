...
const pageTitle = computed(() => isReviewPage.value ? '审核与同步' : isAdmin.value ? '修正记录' : '我的修正')
const pageDescription = computed(() => {
  if (isReviewPage.value) return '集中处理待审核申请，并跟踪同步中与同步失败记录。'
  if (isAdmin.value) return '统一查询全部人工修正记录，并维护尚未审核的申请。'
  return '提交专家、机构/企业及专家任职关系的修正申请，管理员审核后同步。'
})
...
function canCancel(row: CorrectionRecord) {
  return row.status === 'PENDING_REVIEW' && (!isAdmin.value || row.submitterId === String(authStore.profile?.user.id))
}
...
async function cancel(row: CorrectionRecord) {
  if (!window.confirm(`确认撤销“${row.title}”吗？`)) return
  if (dataMode.value === 'example') { row.status = 'CANCELLED'; row.updatedAt = new Date().toISOString(); applyExampleData(); Message.success('示例申请已撤销'); return }
  try { await cancelCorrection(row.id); Message.success('申请已撤销'); await load() } catch (error) { Message.error(errorMessage(error)) }
}
...
<template>
  <div class="correction-page">
    <header class="page-heading"><div><span>{{ isAdmin ? 'ADMIN GOVERNANCE' : 'DATA CORRECTION' }}</span><h1>{{ pageTitle }}</h1><p>{{ pageDescription }}</p></div><div class="page-actions"><button class="page-action" type="button" @click="queryVisible=true">查询</button><button v-if="!isReviewPage" class="page-action" type="button" @click="openCreate">新增修正申请</button></div></header>
    ...
    <a-modal v-model:visible="formVisible" :footer="false" :width="720" :title="editing ? '修改修正申请' : '新增人工修正'">
      <div class="correction-dialog-shell">
        <div class="correction-dialog-body">
          <div class="form-grid"><div class="form-field"><span>修正对象</span><a-select v-model="form.targetType" aria-label="修正对象" class="correction-select" :disabled="Boolean(editing)" :scrollbar="true"><a-option value="expert">专家</a-option><a-option value="organization">机构/企业</a-option><a-option value="relation">专家任职关系</a-option></a-select></div><div class="form-field"><span>操作类型</span><a-select v-model="form.operation" aria-label="操作类型" class="correction-select" :disabled="Boolean(editing)" :scrollbar="true"><a-option value="create">新增</a-option><a-option value="update">修改</a-option><a-option value="delete">删除（软删除）</a-option></a-select></div><label class="wide"><span>对象 ID</span><a-input v-model="form.targetId" :disabled="Boolean(editing)" placeholder="专家 ID、机构 ID 或 source->target@0" /></label><label class="wide"><span>标题</span><a-input v-model="form.title" /></label><label class="wide"><span>修正原因</span><a-textarea v-model="form.reason" :auto-size="{ minRows: 2, maxRows: 4 }" /></label><label><span>修正前数据（JSON）</span><a-textarea v-model="beforeText" :auto-size="{ minRows: 8, maxRows: 14 }" /></label><label><span>修正后数据（JSON）</span><a-textarea v-model="afterText" :disabled="form.operation === 'delete'" :auto-size="{ minRows: 8, maxRows: 14 }" /></label></div>
        </div>
