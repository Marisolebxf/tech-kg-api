...
import {
  cancelCorrection, createCorrection, getCorrection, listCorrections,
  updateCorrection, type CorrectionOperation, type CorrectionRecord,
...
const props = withDefaults(defineProps<{ scope?: 'mine' | 'admin'; initialStatus?: string }>(), {
  scope: 'mine', initialStatus: '',
})
...
const detailFlowNote = computed(() => {
  if (isAdmin.value) return '本页负责查询全部记录，并维护尚未审核的申请。'
...
<template>
  <div class="correction-page">
    <header class="page-heading"><div class="page-actions"><a-button type="primary" @click="queryVisible=true">查询</a-button><button class="page-action" type="button" @click="openCreate">新增修正申请</button></div></header>
    <section class="summary-row">
      <article><span>记录总数</span><strong>{{ total }}</strong></article><article><span>待审核</span><strong>{{ counts.PENDING_REVIEW || 0 }}</strong></article><article><span>同步失败</span><strong>{{ counts.SYNC_FAILED || 0 }}</strong></article><article><span>已完成</span><strong>{{ counts.COMPLETED || 0 }}</strong></article>
    </section>
    ...
    <a-modal v-model:visible="queryVisible" :footer="false" :width="520" title="查询修正记录">
      <div class="query-form">
        <label class="wide query-keyword" for="correction-query-keyword"><span>关键词</span><a-input id="correction-query-keyword" v-model="queryForm.keyword" :max-length="SEARCH_KEYWORD_MAX_LENGTH" placeholder="修正内容、对象 ID、申请人" @press-enter="submitQuery" /></label>
        ...
        <div class="form-field"><span>状态</span><a-select v-model="queryForm.status" aria-label="状态" class="correction-select" :scrollbar="true"><a-option value="">全部</a-option><a-option value="PENDING_REVIEW">待审核</a-option><a-option value="PENDING_SYNC">同步中</a-option><a-option value="SYNC_FAILED">同步失败</a-option><a-option value="COMPLETED">已完成</a-option><a-option value="REJECTED">已驳回</a-option><a-option value="CANCELLED">已撤销</a-option></a-select></div>
      </div>
