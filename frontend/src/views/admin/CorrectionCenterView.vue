  ...
  <div class="correction-page">
    ...
    <a-modal v-model:visible="queryVisible" :footer="false" :width="520" title="查询修正记录">
      <div class="query-form">
        <div class="wide query-keyword"><span>关键词</span><a-input v-model="queryForm.keyword" aria-label="关键词" :max-length="SEARCH_KEYWORD_MAX_LENGTH" placeholder="修正内容、对象 ID、申请人" @press-enter="submitQuery" /></div>
    ...
    <a-modal v-model:visible="formVisible" :footer="false" :width="720" :title="editing ? '修改修正申请' : '新增人工修正'">
      <div class="correction-dialog-shell">
        <div class="correction-dialog-body">
          <div class="form-grid"><div class="form-field"><span>修正对象</span><a-select v-model="form.targetType" aria-label="修正对象" class="correction-select" :disabled="Boolean(editing)" :scrollbar="true"><a-option value="expert">专家</a-option><a-option value="organization">机构/企业</a-option><a-option value="relation">专家任职关系</a-option></a-select></div><div class="form-field"><span>操作类型</span><a-select v-model="form.operation" aria-label="操作类型" class="correction-select" :disabled="Boolean(editing)" :scrollbar="true"><a-option value="create">新增</a-option><a-option value="update">修改</a-option><a-option value="delete">删除（软删除）</a-option></a-select></div><div class="wide"><span>对象 ID</span><a-input v-model="form.targetId" aria-label="对象 ID" :disabled="Boolean(editing)" placeholder="专家 ID、机构 ID 或 source->target@0" /></div><div class="wide"><span>标题</span><a-input v-model="form.title" aria-label="标题" /></div><div class="wide"><span>修正原因</span><a-textarea v-model="form.reason" aria-label="修正原因" :auto-size="{ minRows: 2, maxRows: 4 }" /></div><div><span>修正前数据（JSON）</span><a-textarea v-model="beforeText" aria-label="修正前数据（JSON）" :auto-size="{ minRows: 8, maxRows: 14 }" /></div><div><span>修正后数据（JSON）</span><a-textarea v-model="afterText" aria-label="修正后数据（JSON）" :disabled="form.operation === 'delete'" :auto-size="{ minRows: 8, maxRows: 14 }" /></div></div>
        </div>
