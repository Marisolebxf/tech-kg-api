...
export const actualServiceRules: Record<string, ActualServiceRule[]> = {
  ...
  'node-indirect': [make('限定深度间接路径', '路径发现与强度过滤规则', '核心专家及二至三跳路径', '输入核心节点、关系类型、path_depth 和 min_strength', '按 2 或 3 跳枚举简单路径；边强度读取 confidence/score/relation_strength，百分制归一化，路径强度取各边强度几何平均；去重后按强度降序、深度升序排列。', '间接路径、节点及平均/最大路径强度', 'path_depth 仅允许 2 或 3；路径强度 >= min_strength（默认 0.65）')],
  ...
  'paper-cooperation': [make('共同署名论文查询', '论文合作统计规则', '专家、有效作者关系和有效论文', '输入两位专家及可选起止月份', '联结双方作者关系与论文，过滤 status=1 和年份范围；统计主题、年份、级别、被引及共同作者，并按论文质量权重计算影响力。', '合作论文、分布、稳定团队和学术影响力', '稳定团队成员共同论文数 >= 2；平均场馆得分 >= 70 标记高水平发表')],
