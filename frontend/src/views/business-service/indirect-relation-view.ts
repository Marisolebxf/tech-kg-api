...
const graphNodeType = (node: IndirectNode, isCore = false): GraphNodeType => {
  ...
  if (label === "Project" || label === "Patent" || label === "PatentFamily")
    return "project";
  if (label === "Paper" || label === "Journal" || label === "Report")
    return "paper";
...
export const parseRelationTypes = (value: string) =>
  value
    .split(/[、,，;；]/)
    .map((item) => item.trim())
    .filter(Boolean);

export function buildIndirectRelationGraph(
  result: ExpertIndirectRelationResult,
) {
  // 后端已限制最终路径数量；使用完整结果，确保摘要/API 的统计与画布一致。
  const selectedPaths = result.paths;
  const nodeMap = new Map<string, IndirectNode>([
  ...
  const nodes: GraphNodeData[] = Array.from(nodeMap.values()).map((node) => {
    ...
    const y =
      idsAtLevel.length === 1
        ? 215
        : 65 + index * (310 / (idsAtLevel.length - 1));
    return {
      id: node.id,
      label: node.name,
...
export function indirectSummaryRows(result: ExpertIndirectRelationResult) {
  ...
  return [
    ...
    [
      "直接关联节点",
      `${directNames}（共 ${result.directNodeCount} 个）`,
    ] as const,
    [
      "间接关联节点",
