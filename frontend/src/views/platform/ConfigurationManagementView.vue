...
<template>
  <div class="configuration-page">
    ...
    <section class="summary-grid">
      ...
      <article><i class="purple">↗</i><div><span class="default-summary-label">默认配置</span><strong class="default-summary-value" :class="{ 'is-unset': !activeDefaultConfig }">{{ activeDefaultConfig?.name || '未设置' }}</strong><small class="default-summary-hint">下次调用生效</small></div></article>
    </section>
...
<style scoped>
.default-summary-value.is-unset{display:inline-flex;align-items:center;gap:6px;color:#86909c;font-size:14px;line-height:22px;font-weight:400}.default-summary-value.is-unset::before{display:block;flex:0 0 6px;width:6px;height:6px;border-radius:50%;background:#86909c;content:""}
...
.category-nav>section{margin:auto 16px 16px;padding:16px;border-radius:6px}.category-nav>section b{font-size:14px;line-height:22px}.category-nav>section p{font-size:12px;line-height:20px}
...
@media(max-width:1024px){.config-workbench{grid-template-columns:84px minmax(0,1fr)}.category-nav>header span,.category-nav>button span,.category-nav>button em,.category-nav>section{display:none}.category-nav>button{grid-template-columns:20px;justify-content:center;padding:0}}
