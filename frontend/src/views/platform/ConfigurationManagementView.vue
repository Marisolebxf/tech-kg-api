...
<template>
  <div class="configuration-page">
    ...
    <section class="config-workbench">
      <aside aria-label="辅助区域 1" class="category-nav">
      ...
      <main class="config-list">
        <header><div><h2>{{ categories.find(item => item.key === activeCategory)?.label }}</h2><span>{{ visibleItems.length }} 项配置</span></div><nav aria-label="功能导航 1"><input aria-label="搜索名称、标识或地址" v-model="keyword" placeholder="搜索名称、标识或地址" /><select aria-label="选择或输入内容" v-model="statusFilter"><option>全部状态</option><option>正常</option><option>异常</option><option>停用</option></select></nav></header>
    ...
    <aside aria-label="辅助区域 2" v-if="selected" class="detail-drawer">
      ...
      <div class="detail-form">
        <label><span>配置名称</span><input aria-label="输入内容" v-model="selected.name" /></label>
        <label><span>服务类型</span><input aria-label="输入内容" v-model="selected.type" /></label>
        <label v-if="selected.isLlm" class="wide"><span>Base URL</span><input aria-label="https://open.bigmodel.cn/api/paas/v4" v-model="selected.baseUrl" placeholder="https://open.bigmodel.cn/api/paas/v4" /></label>
        <label v-if="selected.isLlm"><span>模型</span><input aria-label="glm-4.7-flash" v-model="selected.model" placeholder="glm-4.7-flash" /></label>
        <label v-if="!selected.isLlm" class="wide"><span>连接地址 / 版本</span><input aria-label="输入内容" v-model="selected.endpoint" /></label>
        <label class="wide"><span>配置说明</span><textarea aria-label="选择或输入内容" v-model="selected.description" /></label>
        <label><span>负责人</span><input aria-label="输入内容" v-model="selected.owner" /></label>
        <label v-if="selected.isLlm"><span>访问凭据</span><input aria-label="输入内容" :value="selected.apiKeyMasked || (selected.hasApiKey ? '••••••••' : '未设置')" readonly /></label>
        <label v-if="selected.isLlm" class="wide"><span>更新 API Key（留空保留原值）</span><input aria-label="输入新 Key 覆盖原值" v-model="selected.apiKey" type="password" placeholder="输入新 Key 覆盖原值" /></label>
      </div>
    ...
    <aside aria-label="辅助区域 3" v-if="dialogOpen" class="create-dialog">
      ...
      <div class="dialog-form">
        <template v-if="isLlmCategory">
          <label class="wide"><span>配置名称 *</span><input aria-label="例如：科技文本抽取大模型" v-model="form.name" placeholder="例如：科技文本抽取大模型" /></label>
          <label class="wide"><span>Base URL *</span><input aria-label="https://open.bigmodel.cn/api/paas/v4" v-model="form.baseUrl" placeholder="https://open.bigmodel.cn/api/paas/v4" /></label>
          <label><span>模型 *</span><input aria-label="glm-4.7-flash" v-model="form.model" placeholder="glm-4.7-flash" /></label>
          <label><span>负责人</span><input aria-label="输入内容" v-model="form.owner" /></label>
          <label class="wide"><span>API Key</span><input aria-label="明文保存到数据库，页面脱敏展示" v-model="form.apiKey" type="password" placeholder="明文保存到数据库，页面脱敏展示" /></label>
          <label class="wide"><span>说明</span><textarea aria-label="说明用途或变更影响" v-model="form.description" placeholder="说明用途或变更影响" /></label>
          <label class="wide checkbox"><input aria-label="选择此项" type="checkbox" v-model="form.isDefault" /><span>设为默认（同一时刻仅一条默认 LLM 生效）</span></label>
        </template>
        <template v-else>
