...
const feedback = ref<{ message: string; tone: 'success' | 'error' } | null>(null)
...
const fillForm = computed(() => ({ titleZh: fillTitleZh.value }))
const fillRules: Rules = {
  titleZh: { required: '请填写论文中文标题' },
}
const { visibleError: fillVisibleError, validate: validateFill, touch: touchFill, clearErrors: clearFillErrors } = useFormValidation(fillForm, fillRules)
...
async function loadReview() {
  ...
  } catch (error) { setFeedback(error instanceof Error ? error.message : '人工处理详情加载失败', 'error') }
...
async function claimCase() {
  ...
  try {
    ...
    record.value = mapProductionRecord(productionCase.value); startHeartbeat(); setFeedback('领取成功，系统已开始保持处理心跳。')
  } catch (error) { setFeedback(error instanceof Error ? `${error.message}，请重新加载。` : '领取失败', 'error') }
}
...
const footerHint = computed(() => {
  ...
  if (templateId.value === 'T_DQ_FILL' && !fillTitleZh.value.trim()) return '请先补全论文中文标题，再保存并重跑'
...
const handleAction = async (action: ReviewAction | { id: string; label: string; kind: string; rerun?: boolean }) => {
  ...
  if (!productionMode && action.id === 'retry-task') {
    try {
      ...
      setFeedback('已重新下发当前任务，人工处理项保持待处理，可在任务中心查看执行进度。')
    } catch (error) {
      setFeedback(error instanceof Error ? error.message : '任务重试失败', 'error')
    }
    return
  }
  if (!productionMode && action.id === 'skip-task') {
    try {
      ...
      setFeedback('任务已撤销，不再进入下游流程。')
    } catch (error) {
      setFeedback(error instanceof Error ? error.message : '任务撤销失败', 'error')
    }
    return
  }
  ...
  try {
    ...
    setFeedback(error instanceof Error ? error.message : '人工处理提交失败', 'error')
  }
}

const runPrimary = () => {
  if (productionMode) {
    if (!preferredProductionAction.value || hasUnknownComponent.value) return
    if (dynamicResult.value.entityVerdict === 'merge' && !String(dynamicResult.value.targetEntityId || '').trim()) {
      setFeedback('选择合并裁决时需填写目标实体 ID', 'error')
      return
    }
  ...
  if (templateId.value === 'T_DQ_FILL' && !validateFill()) {
    setFeedback('请先补全论文中文标题，再保存并重跑', 'error')
    return
  }
...
<template>
    ...
    <main class="rw-body">
      ...
      <section v-else-if="templateId === 'T_DQ_FILL'" class="zone zone-fill">
        ...
        <div class="fill-form">
          ...
          <FormField class="wide-field" label="title_zh" required :error="fillVisibleError('titleZh')">
            <input v-model="fillTitleZh" :disabled="!isEditable" placeholder="论文中文标题" @blur="touchFill('titleZh')" />
          </FormField>
      ...
      <p v-if="feedback" :class="['rw-feedback', `rw-feedback--${feedback.tone}`]">{{ feedback.message }}</p>
    </main>
  ...
  <div v-else-if="loading" class="rw-empty">
    <h1>加载中…</h1>
  </div>
...
.map-row {
  display: grid;
  grid-template-columns: minmax(180px, 0.9fr) minmax(220px, 1fr) minmax(300px, 1.3fr);
  gap: 12px;
  align-items: center;
}
