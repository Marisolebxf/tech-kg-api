...
let initialReviewTab = productionMode ? '我的待办' : '待处理'
if (route.query.tab === 'history') {
  initialReviewTab = '历史记录'
}
const reviewTab = ref(initialReviewTab)
...
const filteredReviewRows = computed(() => {
  const rows = reviewRows.value.filter((row) => {
    ...
    const inTab = productionMode
      || (reviewTab.value === '历史记录' ? row.status !== '待处理' : row.status === '待处理')
    return inTab
...
watch(() => route.query.tab, (value) => {
  if (value === 'history') {
    reviewTab.value = '历史记录'
  } else {
    reviewTab.value = productionMode ? '我的待办' : '待处理'
  }
  status.value = '全部状态'
})
  ...
  try {
    if (productionMode) {
      ...
      reviewRecords.value = response.items.map((row: ProductionReviewCase) => ({
        id: row.id, batch: row.batchId || '-', module: row.phase, node: row.nodeId, type: row.errorType, category: row.category, domain: row.domain, objectType: row.objectType, objectId: row.objectId, object: row.objectName, ruleId: row.templateId, evidence: `${row.evidence?.length || 0} 项`, score: row.riskLevel, handler: row.assigneeName || '待领取', status: reviewRecordStatus(row.status), updatedAt: row.updatedAt, sourceResult: row.diagnosis, suggestion: row.scope, sourceTable: row.sourceTable || '-', sourceRecordId: row.sourceRecordId || '-', confidenceValue: row.riskLevel, confidenceLabel: row.status,
      }))
    } else {
...
<template>
  <div class="ops-page">
    ...
    <section class="ops-panel">
      <div v-if="mode === 'alerts'" class="alert-tabs"><nav aria-label="功能导航 1"><button v-for="item in alertCategories" :key="item" type="button" :class="{ active:alertCategory===item }" @click="alertCategory=item">{{ item }}</button></nav><label><input aria-label="选择此项" v-model="blockingOnly" type="checkbox" />仅看已阻断</label></div>
      ...
      <div :class="['ops-filter', { 'is-review': mode === 'review' }]">
        <input aria-label="mode === 'review' ? '搜索处理实例 ID、对象或来源记录' : '搜索批次、对象、异常原因'" v-model="keyword" :placeholder="mode === 'review' ? '搜索处理实例 ID、对象或来源记录' : '搜索批次、对象、异常原因'" />
        <select aria-label="选择或输入内容" v-if="mode === 'alerts'" v-model="severity"><option>全部风险</option><option>高风险</option><option>中风险</option><option>低风险</option></select>
        <select aria-label="选择或输入内容" v-else v-model="batchFilter"><option>全部更新批次</option><option v-for="item in [...new Set(reviewRows.map(row => row.batch))]" :key="item">{{ item }}</option></select>
        <select aria-label="选择或输入内容" v-if="mode === 'alerts'" v-model="domain"><option>全部业务域</option><option>人才域</option><option>论文域</option><option>企业域</option></select>
        <select aria-label="选择或输入内容" v-if="mode === 'alerts'" v-model="status"><option>全部状态</option><option>待处理</option><option>处理中</option><option>已关闭</option></select>
        <button type="button" @click="resetFilters">清空筛选</button>
      </div>

      <div v-if="mode === 'alerts'" class="ops-table-scroll"><table aria-label="数据表">
      ...
      <div v-else class="ops-review-table-scroll"><table aria-label="数据表">
        <thead>
          <tr>
            <th>处理实例 ID</th>
            <th>待处理对象</th>
      ...
      <footer v-if="mode === 'alerts'" class="alert-pagination"><span>每页显示　<select aria-label="每页显示条数"><option>20</option><option>50</option><option>100</option></select>　共 158 条异常</span><nav aria-label="功能导航 3"><button type="button" disabled>上一页</button><button class="active" type="button">1</button><button type="button">2</button><button type="button">3</button><button type="button">…</button><button type="button">8</button><button type="button">下一页</button></nav></footer>
      <footer v-else class="review-pagination"><span>共 {{ productionMode ? reviewTotal : filteredReviewRows.length }} 条</span><nav aria-label="功能导航 4"><button type="button" disabled>上一页</button><button class="active" type="button">1</button><button type="button">下一页</button></nav></footer>
    </section>
...
<style scoped>
.ops-page{display:flex;box-sizing:border-box;height:100%;min-height:0;overflow:hidden;padding-bottom:2px;color:#16233b;flex-direction:column}.ops-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:14px}.ops-head h1{margin:0;font-size:20px}.ops-head p{margin:3px 0 0;color:#61708a;font-size:13px}.primary,.ops-filter button{height:34px;padding:0 16px;border:0;border-radius:6px;background:#004ecc;color:#fff;cursor:pointer}.ops-metrics{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin-bottom:14px}.ops-metrics.is-alerts{grid-template-columns:repeat(6,minmax(0,1fr))}.ops-metrics article{display:grid;gap:7px;padding:18px;border:1px solid #bdd7ff;border-radius:9px;background:linear-gradient(145deg,#fff,#f2f8ff);box-shadow:0 8px 20px rgba(48,105,194,.09)}.ops-metrics span{color:#61708a;font-size:14px}.ops-metrics strong{font-size:27px}.ops-metrics em{color:#7890b5;font-size:12px;font-style:normal}.ops-feedback{margin:-3px 0 12px;padding:9px 12px;border:1px solid #b2ccff;border-radius:6px;background:#f0f5ff;color:#344f7a;font-size:12px}.ops-panel{display:flex;flex:1;min-height:0;overflow:hidden;border:1px solid #bdd7ff;border-radius:9px;background:rgba(255,255,255,.94);box-shadow:0 12px 28px rgba(48,105,194,.1);flex-direction:column}.ops-filter{display:grid;grid-template-columns:minmax(260px,1fr) repeat(3,160px) auto;gap:10px;padding:14px;border-bottom:1px solid #dce9ff;background:#f7fbff}.ops-filter input,.ops-filter select{height:34px;padding:0 10px;border:1px solid #bdd7ff;border-radius:6px;background:#fff;color:#273957}.ops-table-scroll{max-height:430px;overflow:auto}table{width:100%;border-collapse:collapse;font-size:13px}th,td{height:52px;padding:10px 14px;border-bottom:1px solid #e5edf8;text-align:left;white-space:nowrap}th{position:sticky;z-index:2;top:0;background:#f4f8fd;color:#5a6c88;font-weight:600}td{color:#273957}td small{display:block;margin-top:4px;color:#7b89a1}.alert-reason{min-width:240px;white-space:normal;line-height:19px}.alert-actions{display:grid;gap:5px}.alert-tabs{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:0 14px;border-bottom:1px solid #dce9ff}.alert-tabs nav{display:flex;overflow:auto}.alert-tabs button{padding:13px 14px;border:0;border-bottom:2px solid transparent;background:transparent;color:#52647f;white-space:nowrap;cursor:pointer}.alert-tabs button.active{border-color:#004ecc;color:#004ecc;font-weight:600}.alert-tabs label{color:#5f6f88;font-size:12px;white-space:nowrap}.alert-tabs input{margin-right:6px}.level-严重,.level-警告,.level-提示{width:max-content;padding:2px 7px;border-radius:10px}.level-严重{background:#fee4e2!important;color:#b42318!important}.level-警告{background:#fff3d8!important;color:#93370d!important}.level-提示{background:#eaf2ff!important;color:#004ecc!important}.alert-status{padding:3px 8px;border-radius:10px;background:#edf2f7;color:#52647f}.alert-status.is-待处理{background:#fff0e8;color:#c4320a}.alert-status.is-处理中{background:#eaf2ff;color:#004ecc}.alert-status.is-已关闭{background:#e9f8ef;color:#067647}.alert-pagination{display:flex;align-items:center;justify-content:space-between;padding:12px 14px;background:#fff;color:#475467;font-size:12px}.alert-pagination nav{display:flex;gap:5px}.alert-pagination button,.alert-pagination select{height:29px;padding:0 10px;border:1px solid #d3deee;border-radius:4px;background:#fff;color:#52647f}.alert-pagination button.active{border-color:#004ecc;color:#004ecc}.alert-pagination button:disabled{opacity:.45}td a,.link{border:0;background:transparent;color:#004ecc;cursor:pointer;text-decoration:none}.danger{color:#b42318}.success{color:#067647}@media(max-width:1280px){.ops-metrics.is-alerts{grid-template-columns:repeat(3,1fr)}}@media(max-width:1100px){.ops-metrics{grid-template-columns:repeat(2,1fr)}.ops-filter{grid-template-columns:1fr 1fr}.ops-panel{overflow:hidden}}
.review-context{display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:12px;padding:12px 14px;border:1px solid #b2ccff;border-radius:7px;background:#f0f5ff}.review-context div{display:grid;gap:3px}.review-context strong{font-size:13px}.review-context span{color:#65738b;font-size:11px}.review-context a{color:#004ecc;font-size:12px;text-decoration:none;white-space:nowrap}
...
.link-disabled{color:#59636f;font-size:12px;cursor:default}
...
.review-mask{position:fixed;z-index:49;inset:0;border:0;background:rgba(16,36,76,.24)}.review-drawer{position:fixed;z-index:50;top:0;right:0;display:grid;grid-template-rows:auto minmax(0,1fr) auto;width:620px;height:100vh;background:#f8fbff;box-shadow:-18px 0 42px rgba(34,74,132,.22)}.review-drawer>header{display:flex;justify-content:space-between;padding:20px;border-bottom:1px solid #dce8f8;background:#fff}.review-drawer header span{color:#004ecc;font-size:11px}.review-drawer h2{margin:6px 0 3px;font-size:19px}.review-drawer header p{margin:0;color:#70809a;font-size:12px}.review-drawer header>button{width:30px;height:30px;border:0;border-radius:5px;background:#f0f4fa;font-size:20px;cursor:pointer}.review-body{overflow:auto;padding:16px}.review-body section,.review-compare article{padding:14px;border:1px solid #dce8f8;border-radius:7px;background:#fff}.review-body h3{margin:0 0 8px;font-size:14px}.review-body p,.review-body li{color:#61708a;font-size:12px;line-height:20px}.review-compare{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:12px 0}.review-compare span{display:block;margin-bottom:9px;color:#70809a;font-size:11px}.review-compare strong{font-size:13px}.review-compare em{color:#b42318;font-size:11px;font-style:normal}.review-compare input,.review-compare textarea{width:100%;padding:8px;border:1px solid #bdd0ea;border-radius:5px;font:inherit}.review-compare textarea{min-height:90px;margin-top:8px;resize:vertical}.review-success{padding:10px 12px;border:1px solid #a6f4c5;border-radius:6px;background:#ecfdf3!important;color:#067647!important}.review-drawer>footer{display:flex;justify-content:flex-end;gap:8px;padding:13px 16px;border-top:1px solid #dce8f8;background:#fff}.review-drawer>footer button{height:34px;padding:0 13px;border:1px solid #bdd0ea;border-radius:6px;background:#fff;color:#40516d;cursor:pointer}.review-drawer>footer .primary{border-color:#004ecc;background:#004ecc;color:#fff}@media(max-width:720px){.review-drawer{width:94vw}.review-compare{grid-template-columns:1fr}}
.ops-head,.ops-metrics,.ops-feedback,.review-context{flex:0 0 auto}.alert-tabs,.ops-filter,.alert-pagination,.review-tabs,.review-pagination{flex:0 0 auto}.ops-table-scroll,.ops-review-table-scroll{flex:1;min-height:0;max-height:none;overflow:auto}.ops-filter.is-review{grid-template-columns:minmax(280px,1fr) 170px 220px auto}.ops-review-table-scroll table{min-width:1900px}.review-tabs{display:flex;gap:4px;padding:0 14px;border-bottom:1px solid #dce9ff;background:#fff}.review-tabs button{display:flex;align-items:center;gap:7px;padding:13px 15px 11px;border:0;border-bottom:2px solid transparent;background:transparent;color:#60708a;cursor:pointer}.review-tabs button.active{border-color:#004ecc;color:#004ecc;font-weight:600}.review-tabs em{padding:1px 7px;border-radius:99px;background:#edf3fc;color:#52627a;font-size:10px;font-style:normal}.review-tabs button.active em{background:#e6efff;color:#004ecc}.review-pagination{display:flex;align-items:center;justify-content:space-between;padding:11px 14px;border-top:1px solid #e4ecf6;background:#fff;color:#52627a;font-size:11px}.review-pagination nav{display:flex;gap:5px}.review-pagination button{height:28px;padding:0 10px;border:1px solid #d3deee;border-radius:4px;background:#fff;color:#52647f}.review-pagination button.active{border-color:#004ecc;color:#004ecc}.review-pagination button:disabled{opacity:.45}.review-empty{height:100px!important;color:#52627a;text-align:center!important}.ops-review-table-scroll td code{color:#004ecc;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}
.review-type-cell{min-width:170px}.review-type-cell>span{display:inline-flex;padding:3px 9px;border-radius:99px;background:#eaf2ff;color:#004ecc;font-size:11px}.review-type-cell>span.is-low-confidence{background:#fff3d8;color:#b54708}.review-type-cell>span.is-extraction{background:#fef3f2;color:#b42318}.review-type-cell>span.is-schema{background:#edf0ff;color:#444ce7}.review-type-cell>span.is-normalization{background:#ecfdf3;color:#067647}.review-type-cell>span.is-other{background:#f2f4f7;color:#475467}
.ops-review-table-scroll table{min-width:1900px}.review-risk-explain{display:grid;flex:0 0 auto;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-bottom:12px}.review-risk-explain>div{display:grid;gap:3px;padding:11px 14px;border:1px solid #b9d2f4;border-radius:7px;background:#f7fbff}.review-risk-explain strong{color:#344861;font-size:11px}.review-risk-explain span{color:#6d7c93;font-size:9px;line-height:16px}.review-confidence-cell{min-width:130px;white-space:normal}.review-confidence-cell>b{display:inline-block;margin-right:7px;font-size:13px}.review-confidence-cell>small{display:inline;color:#718098}.review-confidence-cell>span{display:block;margin-top:4px;color:#78869b;font-size:9px;line-height:15px}@media(max-width:900px){.review-risk-explain{grid-template-columns:1fr}}
.alert-impact{min-width:260px;max-width:340px;white-space:normal}.alert-impact strong,.alert-impact small{display:block;line-height:18px}.alert-impact small{color:#52627a}.review-risk-explain>div:first-child{border-color:#f5b8b3;background:#fff5f4}.review-risk-explain>div:first-child strong{color:#b42318}.review-risk-explain>div:nth-child(2){border-color:#f3d08a;background:#fffaf0}.review-risk-explain>div:nth-child(2) strong{color:#b54708}.review-type{min-width:150px}.review-confidence-cell>em{display:block;width:max-content;margin-top:5px;padding:2px 7px;border-radius:9px;background:#fff3d8;color:#b54708;font-size:9px;font-style:normal}
.level-高风险{width:max-content;padding:2px 7px;border-radius:10px;background:#fee4e2;color:#b42318}.level-中风险{width:max-content;padding:2px 7px;border-radius:10px;background:#fff3d8;color:#b54708}.level-低风险{width:max-content;padding:2px 7px;border-radius:10px;background:#eaf2ff;color:#004ecc}
...
.review-cat-bar button.active{border-color:#004ecc;background:#eef4ff;color:#004ecc;font-weight:600}
...
.review-cat-bar button.cat.is-normalization.active{border-color:#067647;background:#ecfdf3;color:#067647}
...
.review-question-cell small{margin-top:4px;color:#004ecc}
.scope-batch{color:#b42318!important}
.scope-task{color:#004ecc!important}
