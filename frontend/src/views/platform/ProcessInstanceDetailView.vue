...
type StepExecutionField = 'status' | 'count' | 'abnormal' | 'duration'
type Step = {
  id: string
  phase: '数据处理' | '图谱构建'
  name: string
...
const baseSteps: Omit<Step, StepExecutionField>[] = [
...
const steps = computed<Step[]>(() => {
  if (processingInstance.value?.steps?.length) {
    return processingInstance.value.steps.map((step) => {
      let defaultRisk: RiskLevel = '低风险'
      if (step.status === '需人工处理') {
        defaultRisk = '高风险'
      } else if (step.phase === '图谱构建') {
        defaultRisk = '中风险'
      }
      return {
        ...step,
        risk: step.risk ?? defaultRisk,
        engine: step.engine ?? (step.phase === '图谱构建' ? 'Temporal KG Worker' : 'Temporal Data Worker'),
      }
    })
  }
  const instance = processingInstance.value
  const reviewType = instance?.reviewType ?? ''
  const isDictionaryError = reviewType === '公共字典配置异常'
  const isSchemaError = reviewType === 'Schema 批量映射失败'
  const isModelError = reviewType === '模型批量输出异常'
  const isExecutionError = reviewType === '单任务执行失败'
  const isEntityConflict = reviewType === '实体冲突'
  const isEvidenceReview = ['关系证据不足', '属性冲突', '低置信度'].includes(reviewType)

  let quality: Pick<Step, StepExecutionField> = {
    status: '成功',
    count: instance ? '1 条已检验' : '24,755 通过',
    abnormal: instance?.stage === '数据处理' ? '1' : '385',
    duration: '1分42秒',
  }
  if (isDictionaryError) {
    quality = { status: '待执行', count: '前置节点阻断', abnormal: '-', duration: '-' }
  }

  let llm: Pick<Step, StepExecutionField> = {
    status: '成功',
    count: instance ? '1 个候选结果' : '3,261 实体 / 8,942 关系',
    abnormal: '0',
    duration: '8分24秒',
  }
  if (isSchemaError) {
    llm = { status: '待执行', count: '前置节点阻断', abnormal: '-', duration: '-' }
  } else if (isModelError) {
    llm = { status: '需人工处理', count: '未产生有效结果', abnormal: '批量', duration: '未完成' }
  }

  let align: Pick<Step, StepExecutionField> = {
    status: '待执行', count: '-', abnormal: '-', duration: '-',
  }
  if (isExecutionError || isEntityConflict) {
    align = {
      status: '需人工处理',
      count: isExecutionError ? '未产生结果' : '1 个候选冲突',
      abnormal: '1',
      duration: isExecutionError ? '超时' : '6秒',
    }
  } else if (isEvidenceReview) {
    align = { status: '成功', count: '1 个候选已对齐', abnormal: '0', duration: '6秒' }
  }
  ...
  const resolved = baseSteps.map<Step>((step, index) => {
    if (!needsTaskReview.value) {
      let count = '已完成'
      if (index < 4) {
        count = `${batch.value.input.toLocaleString()} 条`
      } else if (step.id === 'llm') {
        count = `${batch.value.entities.toLocaleString()} 实体 / ${batch.value.relations.toLocaleString()} 关系`
      }
      return {
        ...step,
        status: '成功',
        count,
        abnormal: '0',
        duration: step.risk === '高风险' ? '8分24秒' : '1分08秒',
      }
    }
    const normalize: Pick<Step, StepExecutionField> = isDictionaryError
      ? { status: '需人工处理', count: '1 个异常批次', abnormal: '批量', duration: '未完成' }
      : { status: '成功', count: '24,755 条', abnormal: '0', duration: '4分06秒' }
    const schema: Pick<Step, StepExecutionField> = isSchemaError
      ? { status: '需人工处理', count: '1,284 条受影响', abnormal: '批量', duration: '未完成' }
      : { status: '成功', count: '7 类实体', abnormal: '0', duration: '36秒' }
    const validate: Pick<Step, StepExecutionField> = isEvidenceReview
      ? { status: '需人工处理', count: '1 个候选结果', abnormal: '1', duration: '12秒' }
      : { status: '待执行', count: '-', abnormal: '-', duration: '-' }
    const attention: Record<string, Pick<Step, StepExecutionField>> = {
      source: { status: '成功', count: '25,140 条', abnormal: '0', duration: '2分18秒' },
      incremental: { status: '成功', count: '25,140 条', abnormal: '0', duration: '48秒' },
      normalize,
      quality,
      write: { status: instance?.stage === '数据处理' ? '待执行' : '成功', count: isDictionaryError ? '前置节点阻断' : '-', abnormal: '-', duration: '-' },
      schema,
      llm,
      align,
      validate,
...
const attentionLabel = computed(() => {
  if (selectedStep.value.risk === '高风险') return '重点关注'
  if (selectedStep.value.risk === '中风险') return '一般关注'
  return '常规节点'
})
...
const resultStatus = computed(() => {
  if (isProcessLevelIncident.value) return '流程已阻断'
  if (isTaskExecutionFailure.value) return '未产生结果'
  if (processingInstance.value?.status === '人工处理完成') return '人工确认通过'
  if (needsTaskReview.value) return '结果待确认'
  return '结果已通过'
})
...
const incidentScope = computed(() => {
  if (isProcessLevelIncident.value) return '流程级·高风险（P0）'
  if (needsTaskReview.value) return '任务级·中风险（P1）'
  return '无异常'
})
const blockingStrategy = computed(() => {
  if (isProcessLevelIncident.value) return '阻断当前节点及下游，调整后恢复'
  if (needsTaskReview.value) return '只隔离当前任务，其他任务继续执行'
  return '无需阻断'
})

const modelRoutingResult = computed(() => {
  if (isExecutionInterrupted.value) return '未分流'
  return Number(processingInstance.value?.confidence ?? 0.82) >= 0.9
    ? '自动通过'
    : '转人工审核'
})
...
<template>
  <div class="task-detail-page">
    ...
    <section class="detail-workspace">
      <aside aria-label="辅助区域 1" class="process-sidebar">
      ...
      <main class="step-detail">
        ...
        <nav aria-label="功能导航 1" class="detail-tabs"><button v-for="tab in ([['overview','概况与结果'],['io','输入输出'],['logs','异常与日志'],['lineage','数据溯源']] as const)" :key="tab[0]" type="button" :class="{ active: activeTab === tab[0] }" @click="activeTab = tab[0]">{{ tab[1] }}</button></nav>
...
{{ isExecutionInterrupted ? `${processingInstance?.processedAt || '02:00:04'} ERROR 节点执行中断：${processingInstance?.result}\n${processingInstance?.processedAt || '02:00:05'} WARN  未产生结果，置信度不可用\n${processingInstance?.processedAt || '02:00:06'} INFO  ${blockingStrategy}` : `${processingInstance?.processedAt || '02:00:04'} INFO  节点程序执行成功，已生成输出快照\n${needsTaskReview ? `${processingInstance?.processedAt || '02:00:05'} WARN  输出未通过业务验收：${processingInstance?.result}\n${processingInstance?.processedAt || '02:00:06'} INFO  结果已隔离，等待人工确认` : `${processingInstance?.processedAt || '02:00:05'} INFO  输出通过质量验收，可供下游使用`}` }}</pre></div>
        ...
        <div v-else class="trace-content">
          <section class="lineage-compare">
            ...
            <table aria-label="数据表"><thead><tr><th>来源表</th><th>记录 ID</th><th>原始字段</th><th>原始值</th><th>判定用途</th></tr></thead><tbody><tr v-for="row in lineageEvidenceRows" :key="`${row.table}-${row.record}-${row.field}`"><td><code>{{ row.table }}</code></td><td>{{ row.record }}</td><td><code>{{ row.field }}</code></td><td class="raw-value">{{ row.raw }}</td><td>{{ row.basis }}</td></tr></tbody></table>
          </section>
...
<style scoped>
.task-detail-page{height:100%;overflow:auto;padding:0 0 18px;color:#17233b}.detail-head{display:flex;align-items:flex-end;justify-content:space-between;gap:20px;margin-bottom:12px}.detail-head a{color:#004ecc;font-size:12px;text-decoration:none}.detail-head>div>span{margin-left:10px;color:#8793a8;font-size:10px}.detail-head h1{margin:7px 0 3px;font-size:22px}.detail-head p{margin:0;color:#475467;font-size:11px}.detail-actions{display:flex;gap:8px}.detail-actions button{height:34px;padding:0 14px;border:1px solid #bdd0ea;border-radius:6px;background:#fff;color:#40516d;cursor:pointer}.detail-actions .primary{border-color:#b42318;background:#b42318;color:#fff}.action-message{margin:0 0 12px;padding:9px 12px;border:1px solid #b2ccff;border-radius:6px;background:#f0f5ff;color:#344f7a;font-size:11px}.attention-banner{display:flex;align-items:center;gap:12px;margin-bottom:12px;padding:11px 14px;border:1px solid #f6c7c2;border-radius:8px;background:#fff7f6}.attention-banner>i{display:grid;place-items:center;flex:0 0 28px;width:28px;height:28px;border-radius:50%;background:#b42318;color:#fff;font-style:normal;font-weight:700}.attention-banner>div{flex:1}.attention-banner strong{color:#912018;font-size:12px}.attention-banner p{margin:3px 0 0;color:#77504c;font-size:10px}.attention-banner a{padding:8px 11px;border-radius:5px;background:#b42318;color:#fff;font-size:10px;text-decoration:none}.summary-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-bottom:12px}.summary-grid article{display:grid;gap:5px;padding:13px 14px;border:1px solid #c9dcf7;border-radius:8px;background:#fff}.summary-grid span{color:#6b7890;font-size:10px}.summary-grid strong{font-size:19px}.summary-grid strong.version{font-size:13px}.summary-grid em{overflow:hidden;color:#8793a8;font-size:9px;font-style:normal;text-overflow:ellipsis;white-space:nowrap}.danger{color:#b42318}.warning{color:#b54708}.detail-workspace{display:grid;grid-template-columns:340px minmax(0,1fr);min-height:560px;overflow:hidden;border:1px solid #bed4f3;border-radius:9px;background:#fff}.process-sidebar{border-right:1px solid #dce8f8;background:#f8fbff}.process-sidebar>header{display:flex;align-items:center;justify-content:space-between;padding:13px 14px;border-bottom:1px solid #dce8f8;background:#fff}.process-sidebar h2{margin:0;font-size:14px}.process-sidebar header p{margin:3px 0 0;color:#7b899e;font-size:9px}.process-sidebar header>span{padding:4px 8px;border-radius:999px;background:#eaf2ff;color:#004ecc;font-size:10px}.phase-group{padding:10px}.phase-title{display:flex;align-items:center;justify-content:space-between;padding:4px 3px 8px}.phase-title strong{font-size:11px}.phase-title em{color:#52627a;font-size:9px;font-style:normal}.process-step{display:grid;grid-template-columns:22px minmax(0,1fr) auto;align-items:center;gap:8px;width:100%;min-height:48px;margin-bottom:5px;padding:7px 8px;border:1px solid transparent;border-radius:6px;background:transparent;color:#34435c;text-align:left;cursor:pointer}.process-step:hover,.process-step.active{border-color:#8eb8f7;background:#fff;box-shadow:0 3px 10px rgba(39,89,164,.08)}.process-step>i{display:grid;place-items:center;width:20px;height:20px;border-radius:50%;background:#067647;color:#fff;font-size:10px;font-style:normal}.process-step>span{display:grid;gap:3px}.process-step span strong{display:flex;align-items:center;gap:5px;font-size:11px}.process-step span em{color:#7c899d;font-size:9px;font-style:normal}.process-step span b{padding:1px 5px;border-radius:3px;background:#7f56d9;color:#fff;font-size:8px}.process-step span b.risk{background:#f79009}.process-step>small{color:#475467;font-size:8px}.process-step.is-高风险{min-height:58px;border-left:3px solid #f79009;background:#fffaf0}.process-step.is-需人工处理{border-color:#f7c6c2;background:#fff7f6}.process-step.is-需人工处理>i{background:#b42318}.process-step.is-需人工处理>small{color:#b42318}.process-step.is-待执行{opacity:.62}.process-step.is-待执行>i{background:#59636f}.step-detail{min-width:0}.step-head{display:flex;align-items:flex-start;justify-content:space-between;gap:15px;padding:14px 16px;border-bottom:1px solid #dce8f8;background:#fbfdff}.step-head span{color:#004ecc;font-size:9px}.step-head h2{display:flex;align-items:center;gap:8px;margin:4px 0;font-size:18px}.step-head h2 b{padding:3px 7px;border-radius:4px;background:#7f56d9;color:#fff;font-size:9px}.step-head p{margin:0;color:#52627a;font-size:10px}.step-head>a{padding:8px 11px;border-radius:5px;background:#b42318;color:#fff;font-size:10px;text-decoration:none}.detail-tabs{display:flex;padding:0 14px;border-bottom:1px solid #dce8f8}.detail-tabs button{height:40px;padding:0 14px;border:0;border-bottom:2px solid transparent;background:transparent;color:#475467;font-size:10px;cursor:pointer}.detail-tabs button.active{border-color:#004ecc;color:#004ecc;font-weight:600}.overview-content{display:grid;grid-template-columns:minmax(220px,.75fr) minmax(320px,1.25fr);gap:12px;padding:14px}.overview-content>section{border:1px solid #dce8f8;border-radius:7px;background:#fff}.overview-content h3,.io-content h3,.trace-content h3,.issue-list h3{margin:0;padding:11px 13px;border-bottom:1px solid #e4ecf6;font-size:12px}.metric-card dl,.ai-card dl{display:grid;grid-template-columns:1fr 1fr;margin:0;padding:7px 12px}.metric-card dl{grid-template-columns:1fr}.metric-card dl div,.ai-card dl div{display:flex;justify-content:space-between;gap:10px;padding:7px 3px;border-bottom:1px solid #eef2f7;font-size:10px}.metric-card dt,.ai-card dt{color:#52627a}.metric-card dd,.ai-card dd{margin:0;text-align:right}.ai-card{border-color:#cbbaf7!important;background:#fcfaff!important}.ai-card>header{display:flex;align-items:center;justify-content:space-between;padding:10px 12px;border-bottom:1px solid #e4dcfa}.ai-card header>div{display:flex;align-items:center;gap:8px}.ai-card header b{display:grid;place-items:center;width:27px;height:27px;border-radius:6px;background:#7f56d9;color:#fff;font-size:10px}.ai-card header span{display:grid}.ai-card header strong{font-size:11px}.ai-card header em{color:#766b91;font-size:8px;font-style:normal}.ai-card header>i{padding:3px 7px;border-radius:999px;background:#fff0d5;color:#b54708;font-size:8px;font-style:normal}.result-card{grid-column:1/-1;padding-bottom:10px}.result-card.alert{border-color:#f7c6c2;background:#fff8f7}.result-card.success{border-color:#abefc6;background:#f6fef9}.result-card p,.result-card li{color:#596981;font-size:10px;line-height:18px}.result-card p{margin:10px 13px}.io-content{display:grid;grid-template-columns:1fr 1fr;gap:12px;padding:14px}.io-content>section,.trace-content>section{overflow:hidden;border:1px solid #dce8f8;border-radius:7px}.io-content pre,.log-content>pre{min-height:250px;margin:0;padding:15px;overflow:auto;background:#17233b;color:#d9e7ff;font:10px/18px Consolas,monospace;white-space:pre-wrap}.sample-text,.candidate-result{margin:13px;padding:14px;border-radius:6px;background:#f5f8fc}.sample-text span,.candidate-result span{color:#728099;font-size:9px}.sample-text p{font-size:11px;line-height:20px}.candidate-result strong{display:block;margin:10px 0;font-size:12px}.candidate-result p{color:#68778e;font-size:10px}.candidate-result p b{color:#b54708}.log-content{display:grid;grid-template-columns:230px minmax(0,1fr);gap:12px;padding:14px}.issue-list{overflow:hidden;border:1px solid #f4c7c3;border-radius:7px;background:#fff8f7}.issue-list article{display:grid;gap:7px;padding:14px}.issue-list span,.issue-list em{color:#77504c;font-size:9px;font-style:normal}.issue-list strong{color:#b42318;font-size:24px}.trace-content{display:grid;gap:12px;padding:14px}.lineage{display:flex;align-items:center;justify-content:center;gap:9px;padding:22px;overflow:auto}.lineage span{min-width:115px;padding:12px;border:1px solid #bdd7ff;border-radius:6px;background:#f6faff;font-size:9px;text-align:center}.lineage b{color:#004ecc}.trace-content table{width:100%;border-collapse:collapse;font-size:10px}.trace-content th,.trace-content td{padding:10px 13px;border-bottom:1px solid #e4ecf6;text-align:left}.trace-content th{width:140px;color:#65738b}@media(max-width:1200px){.summary-grid{grid-template-columns:repeat(3,1fr)}.detail-workspace{grid-template-columns:300px minmax(0,1fr)}.overview-content{grid-template-columns:1fr}.result-card{grid-column:auto}}@media(max-width:900px){.summary-grid{grid-template-columns:repeat(2,1fr)}.detail-workspace{grid-template-columns:1fr}.process-sidebar{border-right:0;border-bottom:1px solid #dce8f8}.io-content,.log-content{grid-template-columns:1fr}}
